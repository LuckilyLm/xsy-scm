...
工作区边界：后续 V2 正式代码位于根目录 `xsy-scm-server/` 与 `xsy-scm-web/`；本次 W3 实现仍在
`v2/xsy-scm-v2-server/` 与 `v2/xsy-scm-v2-web/`，仅作为移动前的过渡来源。
...
Backend SCM Business   = SmartAdmin Structure + Legacy Business Rules + C Implementation Reference
```
...
```text
1. v2/docs/architecture/2026-09-15-w3-pricing-legacy-audit.md      ✅ 已完成
2. v2/docs/architecture/2026-09-15-w3-pricing-target-design.md     ← 本文件
...
10. v2/tools/verify_w3_legacy.py + v2/tools/w3_e2e_accounts.py
...
| 25 | `v2/tools/ts_baseline_ratchet.py` | 已有 | **需改**：`SCM_PREFIXES` 已含 `src/views/business/scm/` 与 `src/api/business/scm/` 等；本 W3 新增的 `src/utils/scm-amount.ts` 不在白名单内，**需同步加入 `SCM_PREFIXES`**（该文件由 SCM 交付，必须受「SCM 0 错误」门禁保护，见 §5.2 第 62 项 / §9.6） |
...
| TS 门禁 | `python v2/tools/ts_baseline_ratchet.py check` | – | SCM 区 0 错误；total ≤ 1974 |
...
1. 后端单测：mvn -pl sa-admin test（SCM 全量，W1+W2+W3 一起跑）
   - SCM 测试全在 sa-admin；用 classworlds 启动器 + -Dmaven.multiModuleProjectDirectory=<repo>/v2/xsy-scm-v2-server
...
3. 前端单测：npm run test
4. TS 棘轮：python v2/tools/ts_baseline_ratchet.py check  → 必须 PASS（new 0 / SCM 0 / total ≤ 1974）
...
7. E2E：npx playwright test e2e/scm-pricing.spec.ts（2/2）
   ...
   - 后端 18080 / 前端 18081
8. legacy 冻结复验：python v2/tools/verify_w3_legacy.py
   - legacy 全量 SHA-256 清单（沿用 W2 的 900 文件基线 + 新增 pricing 文件）
   - V6–V9 migration 哈希未变
   - 前端「复制」文件的头部注释包含来源路径
   - 输出 v2/.runtime/w3-legacy-recheck.{manifest,json}
9. 生成 v2/docs/architecture/v3-applied-migrations.sha256（V6–V11）
```
...
T14 e2e/scm-pricing.spec.ts（2 用例）+ v2/tools/w3_e2e_accounts.py

T15 门禁全跑（§9.6 九项）+ v2/tools/verify_w3_legacy.py + v3-applied-migrations.sha256
...
v2/tools/ts_baseline_ratchet.py                             SCM_PREFIXES 加入 src/utils/scm-amount.ts
```

**新增（工具 / 文档）**

```text
v2/tools/verify_w3_legacy.py
v2/tools/w3_e2e_accounts.py
v2/docs/architecture/v3-applied-migrations.sha256
v2/docs/architecture/2026-09-15-w3-pricing-验收报告.md（含 Frontend Migration Provenance）
v2/docs/architecture/2026-09-15-w3-pricing-approval.md
```
...
<!--
  ...
  * - 截图：v2/.runtime/w3-agreement-price-list.png
-->
```
...
**第 3 层：机器校验（`v2/tools/verify_w3_legacy.py`）**
