...
唯一批准差异仍是根 `.gitignore` 新增 `.workbuddy-ai` 与 `v2/.runtime` 两条规则，保留、不还原。
...
W2 新增复验脚本 `v2/tools/verify_w2_legacy.py` 与清单 `v2/docs/architecture/w2-applied-migrations.sha256`，
...
- 只新增 `V8__scm_customer_supplier.sql` 与 `V9__scm_customer_supplier_permissions.sql`；
  不修改 V6/V7；不修改 `module/scm/product/**`；legacy / reference 全部只读；正式代码只写 `v2/**`。
