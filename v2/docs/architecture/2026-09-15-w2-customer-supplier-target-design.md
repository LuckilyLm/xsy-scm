...
> 上游输入：`v2/docs/architecture/2026-09-15-w2-customer-supplier-legacy-audit.md`、`2026-09-15-smartadmin-old-vs-v2-audit-workbuddy.md`
> 冻结基线：HEAD `95a54233586aa3c652e55cd833bb346d51f5a952`，branch `feature/sprint5`
> 写入边界：**仅 `v2/**`**。`xsy-scm-server/**`、`xsy-scm-web/**`、`xsy-scm-miniapp/**`、
> `project-reference-examples/**` 与仓库根 `docs/**` 全部只读。
> 设计日期：2026-09-15　状态：**待人工确认（§12 共 14 项），确认后按 T1 → T14 实施；止于 W2**
>
> **修订记录**
> - `2026-09-15` 初版。
...
| 后端 | `v2/xsy-scm-v2-server/sa-admin/**/module/scm/{common,customer,supplier}/**` |
| 前端 | `v2/xsy-scm-v2-web/src/{api,types,constants,components,views}/**/scm/**` |
| 质量基线 | `v2/docs/quality/ts-baseline.json`、`v2/tools/ts_baseline_ratchet.py`（Step 0 已交付） |
| 报告 | `v2/docs/architecture/2026-09-15-w2-customer-supplier-验收报告.md` |
...
| 项 | 规则 |
|---|---|
| 允许复制 | `project-reference-examples/xsy-scm/xsy-scm-web/src/**` 中**真实存在**且落在 W2 业务范围内的 SCM Vue 页面 / 组件 / 常量 / API 形状，允许复制源码到 `v2/xsy-scm-v2-web` 作为实现起点 |
...
`v2/tools/ts_baseline_ratchet.py` + `v2/docs/quality/ts-baseline.json`：
...
| **Q5** | `warehouse` 主数据是否纳入 W2？ | **推迟到 W3** | 用户 W2 范围未列；W2 无收货/库存消费方（与 W1 排除"可下单查询"的原则一致）；legacy 的 warehouse 规则已在 Legacy Audit §3.4 完整记录，W3 可直接平移 |
