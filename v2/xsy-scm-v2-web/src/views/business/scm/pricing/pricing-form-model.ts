...
export function emptyPrice():PriceForm{return {unitPrice:'',effectiveFrom:'',effectiveTo:null};}
...
export function validateBatch(rows:BatchRow[]):{rowNumber:number;message:string}[] {return rows.flatMap(r=>validatePrice(r,'customerTypeId').map(message=>({rowNumber:r.rowNumber,message})));}
