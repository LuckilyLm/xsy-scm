...
<template>
 <a-form layout="inline" class="smart-query-form" @finish="search">
  <a-form-item label="来源"><a-select v-model:value="query.source" allow-clear style="width:150px" :options="[{value:'AGREEMENT',label:'客户协议价'},{value:'CUSTOMER_TYPE',label:'客户类型价'}]" /></a-form-item>
  <a-form-item label="客户"><CustomerSelect v-model:value="query.customerId" width="180px" /></a-form-item>
  <a-form-item label="客户类型"><CustomerTypeSelect v-model:value="query.customerTypeId" width="150px" /></a-form-item>
  <a-form-item label="SKU"><SkuSelect v-model:value="query.skuId" width="230px" :disabled-statuses="[]" /></a-form-item>
  <a-form-item label="操作"><a-select v-model:value="query.operationType" allow-clear style="width:110px" :options="[{value:'CREATE',label:'新增'},{value:'UPDATE',label:'更新'},{value:'DELETE',label:'删除'}]" /></a-form-item>
  <a-form-item label="有效区间"><a-range-picker v-model:value="effective" show-time value-format="YYYY-MM-DDTHH:mm:ssZ" :allow-empty="[true,true]" /></a-form-item>
  <a-form-item label="操作区间"><a-range-picker v-model:value="operated" show-time value-format="YYYY-MM-DDTHH:mm:ssZ" :allow-empty="[true,true]" /></a-form-item>
  <a-form-item><a-button type="primary" html-type="submit" v-privilege="'scm:pricing:history:query'">查询</a-button></a-form-item>
 </a-form>
 ...
 <a-modal title="价格变更详情" :open="!!selected" :footer="null" :width="760" @cancel="selected=undefined"><a-table :columns="diffColumns" :data-source="diff" row-key="field" :pagination="false" size="small" bordered /></a-modal>
</template>
...
const columns=ref<TableColumnsType<HistoryRow>>([{title:'来源',dataIndex:'source',width:140},{title:'客户',dataIndex:'customerName',width:140},{title:'客户类型',dataIndex:'customerTypeName',width:130},{title:'SKU 编码',dataIndex:'skuCode',width:150},{title:'商品',dataIndex:'productName',width:140},{title:'操作',dataIndex:'operationType',width:90},{title:'操作人',dataIndex:'operator',width:100},{title:'操作时间',dataIndex:'operatedAt',width:210},{title:'当前价格',dataIndex:'currentUnitPrice',align:'right',width:120},{title:'当前生效',dataIndex:'currentEffectiveFrom',width:200},{title:'当前结束',dataIndex:'currentEffectiveTo',width:200},{title:'当前记录',dataIndex:'currentDeleted',width:100},{title:'详情',dataIndex:'action',fixed:'right',width:110}]);
const names:Record<string,string>={id:'价格记录',customerId:'客户',customerTypeId:'客户类型',skuId:'SKU',unitPrice:'单价',effectiveFrom:'生效时间',effectiveTo:'结束时间',version:'版本',deleted:'删除标记'};
const diffColumns=[{title:'字段',dataIndex:'field'},{title:'变更前',dataIndex:'before'},{title:'变更后',dataIndex:'after'}];
const diff=computed(()=>[...new Set([...Object.keys(selected.value?.beforeData||{}),...Object.keys(selected.value?.afterData||{})])].map(k=>({field:names[k]||k,before:String(selected.value?.beforeData?.[k]??'—'),after:String(selected.value?.afterData?.[k]??'—')})));
