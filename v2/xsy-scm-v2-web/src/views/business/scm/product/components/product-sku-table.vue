<template>
  <a-table :columns="columns" :data-source="rows" row-key="skuId" :pagination="false" size="small" :scroll="{ x: 920 }">
    <template #bodyCell="{ column, record }">
      <span v-if="column.dataIndex === 'marketPrice'" class="price">{{ record.marketPrice === null ? '未定价' : `¥ ${record.marketPrice}` }}</span>
      ...
      <span v-else-if="column.dataIndex === 'productType'">{{ record.productType === 'STANDARD' ? '标品' : '非标品' }}</span>
      <span v-else-if="column.dataIndex === 'specValues'">{{ Object.entries(record.specValues || {}).map(([key, value]) => `${key}：${value}`).join(' / ') || '—' }}</span>
    </template>
  </a-table>
</template>
...
const columns: TableColumnsType<ProductSku> = [
  ...
  { title: '市场价', dataIndex: 'marketPrice', width: 130, align: 'right' }, { title: '状态', dataIndex: 'status', width: 80, align: 'center' },
