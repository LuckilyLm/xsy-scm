...
@Service @RequiredArgsConstructor public class AgreementPriceService {
    ...
    public Long add(AgreementPriceAddForm form) {
        ...
        var entity=new AgreementPriceEntity(); apply(entity,form);
    ...
    public void delete(AgreementPriceDeleteForm form) {
        var e=require(form.getAgreementPriceId(),form.getVersion()); String before=snapshot(e);
    ...
    private AgreementPriceEntity require(Long id,Integer version) {
        var e=dao.selectById(id); if(e==null) throw new ScmBusinessException(AGREEMENT_PRICE_NOT_FOUND);
    ...
    private void validateAndLock(AgreementPriceAddForm form,Long exclude) {
        ...
        if(dao.lockParent(form.getCustomerId())==null) { customers.requireTradable(form.getCustomerId()); }
        customers.requireTradable(form.getCustomerId());
        validation.requireSellable(form.getSkuId());
        if(dao.countOverlapping(form.getCustomerId(),form.getSkuId(),form.getEffectiveFrom(),form.getEffectiveTo(),exclude)>0) throw new ScmBusinessException(AGREEMENT_PRICE_OVERLAP);
    }
    private void apply(AgreementPriceEntity e,AgreementPriceAddForm f) {
        e.setCustomerId(f.getCustomerId()); e.setSkuId(f.getSkuId()); e.setUnitPrice(ScmDecimalStrings.parseScale4(f.getUnitPrice()));
    ...
    private String snapshot(AgreementPriceEntity e) {
        var fields=new java.util.LinkedHashMap<String,Object>(); fields.put("id",e.getId());fields.put("customerId",e.getCustomerId());fields.put("skuId",e.getSkuId());
    ...
    private void log(AgreementPriceEntity e,String action,String before) {dao.log(e.getId(),action,ScmOperator.current(),before,snapshot(e));}
}
