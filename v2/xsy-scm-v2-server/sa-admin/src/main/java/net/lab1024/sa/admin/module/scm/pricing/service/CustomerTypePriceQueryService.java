...
@Service @RequiredArgsConstructor public class CustomerTypePriceQueryService {
    private final CustomerTypePriceDao dao;
    public PageResult<CustomerTypePriceVO> query(CustomerTypePriceQueryForm form) {
        if(form.getSortItemList()!=null && form.getSortItemList().stream().anyMatch(i->!java.util.Set.of("effective_from","effective_to","unit_price","updated_at").contains(i.getColumn()))) throw new ScmBusinessException(VALIDATION_ERROR);
    ...
    public CustomerTypePriceVO detail(Long id) {var v=dao.detail(id);if(v==null) throw new ScmBusinessException(CUSTOMER_TYPE_PRICE_NOT_FOUND);return v;}
}
