...
| 正式代码写入根目录 V2 工作区 | — | 全部新增落在 `xsy-scm-server` 与 `xsy-scm-web` | ✅ |
...
C 根目录 = `project-reference-examples/xsy-scm/xsy-scm-web/src`；V2 根目录 = `xsy-scm-web/src`。
...
| `tools/w2_e2e_accounts.py` | `tools/w1_e2e_accounts.py` | W1-derived | 前缀 `w2_e2e_`；权限集 = W1 商品域 401–424（造 SKU 夹具）+ W2 客户/供应商域 431–482 |
| `tools/verify_w2_legacy.py` | `tools/verify_w1_legacy.py` | W1-derived | 同一冻结清单；「已应用 migration」清单扩到 V9 |
...
| TS 基线棘轮 | `python tools/ts_baseline_ratchet.py check` | **PASS**（baseline 1974 / current 1973 / delta −1 / **SCM 0** / new 0） |
...
| legacy SHA-256 复验 | `python tools/verify_w2_legacy.py` | **PASS**（900 文件 / HEAD 未变 / V6–V9 未改 / `.gitignore` 仅批准差异） |
...
approved diff    : .gitignore: .workbuddy-ai + .runtime
...
与 W1 批准基线逐字节一致。W2 新增 `docs/architecture/w2-applied-migrations.sha256`
...
| 文件 | 内容 |
|---|---|
| `.runtime/t14-backend-test.log` | `mvn test` 全量输出（110 例） |
| `.runtime/t14-backend-it.log` | `-Dtest='*IT'` 全量输出（86 例） |
| `.runtime/w2-legacy-recheck.json` | legacy 冻结复验结果（900 文件） |
| `.runtime/w2-customer-detail.png` | 客户详情页截图（E2E 产出） |
| `.runtime/w2-supplier-detail.png` | 供应商详情页截图（E2E 产出） |
| `docs/architecture/w2-applied-migrations.sha256` | V6–V9 应用时哈希基线 |
| `docs/architecture/2026-09-15-w2-customer-supplier-approval.md` | 批准记录与 Q1–Q14 |
| `docs/architecture/2026-09-15-w2-customer-supplier-target-design.md` | Target Design |
| `docs/architecture/2026-09-15-w2-customer-supplier-legacy-audit.md` | Legacy 审计 |
