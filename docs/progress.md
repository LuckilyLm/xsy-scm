...
| 物流配送 L0–L2（V41–V42） | 已实现，编译 / 构建 + 定向集成验证通过；浏览器与真实地图待验收 | 客户 / 仓库定位、订单地理快照、司机车辆、静态排线、规划锁定、取消释放、固定打印；高德配置待补，L3 未开始 |
...
在 `dev-map` 按未提交的静态路线规划实现。新增 V41 / V42，保留现有主线与旧迁移。
...
- Docker Desktop 现有 `postgres:18-alpine` + `redis:7-alpine`；使用单独新建的 `xsy_delivery_verify_20260921` 数据库，Flyway 已升级至 V42。
...
- 测试记录自动回滚；独立测试库已清理，日常开发库未执行本次迁移。后续启动此分支后端时由 Flyway 应用 V41–V42。
