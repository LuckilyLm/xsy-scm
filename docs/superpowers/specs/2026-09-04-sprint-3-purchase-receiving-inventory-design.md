...
本期由用户通过显式命令从已确认销售订单生成需求，不从草稿或待审核订单自动生成。每个活动销售订单行只允许存在一条采购需求；完全相同的重复生成返回已有结果，不同幂等键并发生成时也必须依靠来源行锁和活动唯一索引收敛为同一结果。订单确认后发生退货或其他变化时，本期不自动冲销采购需求，留待后续受控命令处理。
...
一张采购单允许多张收货单；不假定“一单一次收货”。收货单创建时只保存草稿，不改变库存；收货确认才产生库存副作用。

每个收货行必须属于当前采购单的采购单行，且本次收货数量大于零。服务端根据采购单行计划数量、历史确认收货数量和本次数量计算剩余量，严格拒绝任何超收。
...
确认后收货单、收货行和称重记录只读；本期不实现反收货、超收容差或少收关单。累计实收量严格等于采购计划量时采购单转为 `RECEIVED`，大于零但尚未达到计划量时转为 `PARTIALLY_RECEIVED`。
