...
- 状态：后端与管理端已实现；后端已验证，前端验证待本次交付记录，Sprint 3 浏览器 E2E 待补
...
当前交付状态以 `docs/superpowers/verification/2026-09-07-sprint-3-verification.md` 的最新命令证据为准。实现与自动化验证完成并不替代尚缺失的 Sprint 3 浏览器主链路验收。

- [x] 采购需求生成并发安全；
- [x] 采购需求分配使用聚合增量校验；
- [x] 保留采购行不允许替换 demandId；
- [x] 一个采购订单只能创建一个活动收货单；
- [x] 收货单支持多次不可变确认；
- [x] 收货单状态正确流转；
- [x] 采购订单状态正确流转；
...
- [x] 收货与库存事务原子；
- [x] 库存余额与流水一致；
- [x] `PURCHASE_IN` 流水追加且不可变；
- [x] 前端页面、API、路由和导航已实现；
- [x] 后端聚焦测试通过；
- [x] 后端完整测试通过；
- [x] 前端 lint 通过；
- [x] 前端 typecheck 通过；
- [x] 前端 test 通过（13 个测试文件、35 个测试）；
- [x] 前端 build 通过；
- [ ] Sprint 3 关键 E2E 通过（当前仓库尚无对应 Playwright 用例）；
- [x] 代码未覆盖或破坏用户已有未提交修改；
- [x] 变更后的 Git diff 已检查；
- [x] 未验证的项目已明确记录。