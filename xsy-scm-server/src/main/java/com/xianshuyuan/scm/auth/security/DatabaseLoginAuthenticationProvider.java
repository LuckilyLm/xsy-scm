...
public class DatabaseLoginAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        Outcome outcome = transactions.execute(status -> authenticateLocked(username, presentedPassword));
    ...
    private Outcome authenticateLocked(String username, String presentedPassword) {
        ...
        if (user == null) {
            ...
            recordLog(null, username, "FAILURE", "INVALID_CREDENTIALS");
            return Outcome.failure(null, false);
        }
        if (!"ENABLED".equals(user.getStatus()) || user.getPasswordHash() == null) {
            passwords.matches(presentedPassword, dummyPasswordHash);
            recordLog(user.getId(), user.getUsername(), "FAILURE", "ACCOUNT_UNUSABLE");
        ...
        if (user.getLockedUntil() != null && user.getLockedUntil().isAfter(now)) {
            passwords.matches(presentedPassword, dummyPasswordHash);
            recordLog(user.getId(), user.getUsername(), "LOCKED", "ACCOUNT_LOCKED");
            return Outcome.failure(user.getUsername(), false);
        }
        if (!passwords.matches(presentedPassword, user.getPasswordHash())) {
            ...
            recordLog(user.getId(), user.getUsername(), newlyLocked ? "LOCKED" : "FAILURE",
                    newlyLocked ? "LOGIN_FAILURE_THRESHOLD" : "INVALID_CREDENTIALS");
            if (newlyLocked) {
        ...
        recordLog(user.getId(), user.getUsername(), "SUCCESS", null);
        UserDetails details = userDetailsService.loadUserByUsername(user.getUsername());
        return Outcome.success(UsernamePasswordAuthenticationToken.authenticated(
                details, null, details.getAuthorities()));
    }

    private void recordLog(Long userId, String username, String result, String reason) {
        try {
            loginLogs.append(userId, username, result, reason, null, null);
        } catch (RuntimeException ignored) {
