...
public class UserService {
    ...
    public UserResponse detail(long id) { return converter.toResponse(require(users.selectById(id))); }
    ...
    public UserResponse create(CreateUserRequest request, Authentication actor) {
        ...
        user.setUsername(username); user.setDisplayName(request.getDisplayName().strip());
        user.setDepartmentId(request.getDepartmentId()); user.setEmail(email(request.getEmail())); user.setPhone(optional(request.getPhone()));
        user.setPasswordHash(passwords.encode(request.getPassword()));
        user.setAdministrator(false); user.setMustChangePassword(true); user.setStatus("ENABLED");
        user.setAuthVersion(0L); user.setFailedLoginCount(0); user.setVersion(0); user.setDeleted(false);
        user.setCreatedBy(actor.getName()); user.setUpdatedBy(actor.getName());
        user.setCreatedAt(OffsetDateTime.now()); user.setUpdatedAt(user.getCreatedAt());
        users.insert(user);
    ...
    public UserResponse update(long id, UpdateUserRequest request, Authentication actor) {
        ...
        user.setDisplayName(request.displayName().strip()); user.setDepartmentId(request.departmentId());
        user.setEmail(email(request.email())); user.setPhone(optional(request.phone())); user.setUpdatedBy(actor.getName());
        if(users.updateProfile(user)!=1) throw new BusinessException(ErrorCode.DATA_CONFLICT);
        var result=detail(id);
        audit(id,actor,"USER_UPDATE",before,result); invalidateAfterCommit(user.getUsername());
        return result;
    }
    ...
    public UserResponse changeStatus(long id, UserStatusRequest request, Authentication actor) {
        ...
        var result=detail(id);
        audit(id,actor,"USER_STATUS",before,result); invalidateAfterCommit(user.getUsername());
        return result;
    }
    ...
    public void delete(long id, int version, Authentication actor) {
        users.lockSecurityWrites();
        var user=locked(id,version); protect(user,actor,true);
        if(users.updateState(id,version,user.getStatus(),true,actor.getName())!=1)
            throw new BusinessException(ErrorCode.DATA_CONFLICT);
        audit(id,actor,"USER_DELETE",converter.toResponse(user),null); invalidateAfterCommit(user.getUsername());
    }
    ...
    private void department(Long id) {
        if(id!=null && users.lockEnabledDepartment(id)==null) throw new BusinessException(SystemErrorCodes.INVALID_DEPARTMENT);
    }
    ...
    private void audit(long id,Authentication actor,String operation,UserResponse before,UserResponse after) {
        ...
        try { users.insertAudit(id,actorId,actor.getName(),operation,json.writeValueAsString(before),json.writeValueAsString(after)); }
        catch(JsonProcessingException failure) { throw new IllegalStateException("Cannot serialize user audit",failure); }
    }
    private String optional(String text) { return text==null || text.isBlank()?null:text.strip(); }
    private String email(String text) { String value=optional(text); return value==null?null:value.toLowerCase(Locale.ROOT); }
}
