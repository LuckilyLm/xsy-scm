...
public class UserRoleGrantService {
    ...
    public UserRolesResponse replace(long id, ReplaceUserRolesRequest request, Authentication authentication) {
        ...
        for (long userId : new TreeSet<>(List.of(id, principal.userId()))) locked.put(userId, users.lockActiveUser(userId));
        ...
        if (!administrator) {
            for (var role : grants.roles(id)) {
                if (!requested.contains(role.getId()) && com.xianshuyuan.scm.auth.security.AuthorityRules.isReservedRole(role.getCode(), Boolean.TRUE.equals(role.getSystemRole()))) forbidden();
            }
            if (Boolean.TRUE.equals(target.getAdministrator())
                    || Set.of("admin", "administrator", "system", "root").contains(target.getUsername().toLowerCase(Locale.ROOT))) forbidden();
            for (long roleId : requested) {
                var role = roles.selectById(roleId);
                if (com.xianshuyuan.scm.auth.security.AuthorityRules.isReservedRole(role.getCode(), Boolean.TRUE.equals(role.getSystemRole()))) forbidden();
            }
        ...
        } catch (JsonProcessingException failure) { throw new IllegalStateException("Cannot serialize user role audit", failure); }
        String username = target.getUsername();
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            @Override public void afterCommit() {
                try { sessions.invalidateAll(username); }
                catch (RuntimeException failure) { log.warn("Session invalidation failed after user role update; account version checks remain active"); }
            }
        });
        return after;
    }
    ...
    private void forbidden() { throw new BusinessException(SystemErrorCodes.PROTECTED_USER); }
}
