...
public final class AfterSalesRules {
    ...
    public static void requireWithinActual(BigDecimal actual, BigDecimal reserved, BigDecimal requested) {
        ...
        if (actual == null || reserved.add(requested).compareTo(actual) > 0)
            throw new AfterSalesRuleException("退货数量超过可退数量");
    }

    public static void requirePositiveApproval(List<BigDecimal> quantities) {
        if (quantities.stream().noneMatch(value -> value != null && value.signum() > 0))
            throw new AfterSalesRuleException("至少一行批准数量必须大于零");
    }
