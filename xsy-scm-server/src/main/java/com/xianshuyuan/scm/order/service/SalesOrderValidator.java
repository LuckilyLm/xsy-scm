...
public class SalesOrderValidator {
    public void validateDraft(SalesOrderSaveRequest request) {
        ...
        for (var item:request.items()) {
            ...
            if (item.manualPriceOverride() && (blank(item.unitPrice()) || blank(item.overrideReason()))) fail(OrderErrorCodes.OVERRIDE_REASON_REQUIRED);
            if (!item.manualPriceOverride() && item.unitPrice()!=null) fail(OrderErrorCodes.INVALID_PRICE_OVERRIDE);
            if (item.unitPrice()!=null && new BigDecimal(item.unitPrice()).signum()<0) fail(OrderErrorCodes.INVALID_PRICE);
        }
    }
    public void requireReason(String reason, ErrorCode code){if(blank(reason))fail(code);}
