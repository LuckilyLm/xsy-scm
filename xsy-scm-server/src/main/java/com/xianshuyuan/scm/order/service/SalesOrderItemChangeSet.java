...
public record SalesOrderItemChangeSet(List<SalesOrderItemEntity> inserted, List<SalesOrderItemEntity> updated,
    ...
    public SalesOrderItemChangeSet {
    ...
    public static SalesOrderItemChangeSet between(long orderId, List<SalesOrderItemEntity> existing, List<SalesOrderItemEntity> requested) {
        ...
        existing.forEach(item -> {
            if (Objects.equals(orderId, item.getOrderId())) unmatched.put(item.getId(), item);
        });
        ...
        for (SalesOrderItemEntity item : requested) {
            ...
            if (!Objects.equals(item.getVersion(), persisted.getVersion()))
