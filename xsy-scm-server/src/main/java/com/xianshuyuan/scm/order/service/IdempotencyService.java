...
public class IdempotencyService {
    ...
    public IdempotencyService(IdempotencyRecordMapper mapper, ObjectMapper json) {
        this.mapper = mapper;
        this.json = json;
        this.hasher = new IdempotencyRequestHasher(json);
    }
    ...
    public Claim claim(String scope, String key, Object request) {
        ...
        IdempotencyGuard.requireMatching(stored.getRequestHash(), hash);
        return new Claim(stored, stored.getResultData() != null);
    }

    public void complete(Claim claim, String type, long resultId, Object result) {
        var row = claim.record();
        row.setResultType(type);
        row.setResultId(resultId);
        row.setResultData(json.valueToTree(java.util.Collections.singletonMap("value", result)));
    ...
    public <T> T replay(Claim claim, Class<T> type) {
        try {
            var value = claim.record().getResultData().get("value");
            if (value == null || value.isNull()) return null;
            return json.treeToValue(value, type);
        } catch (Exception e) {
            throw new IllegalStateException("无法读取幂等结果", e);
        }
    }
