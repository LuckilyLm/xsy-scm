...
(
    ...
    CONSTRAINT ck_purchase_demand_status CHECK (status IN ('PENDING', 'PARTIALLY_ALLOCATED', 'ALLOCATED', 'FULFILLED',
                                                           'CANCELLED')),
...
(
    ...
    CONSTRAINT ck_purchase_receipt_item_weight_fields CHECK (
        (actual_weight IS NULL AND weight_unit IS NULL AND weighing_source IS NULL)
            OR (actual_weight IS NOT NULL AND weight_unit IS NOT NULL AND btrim(weight_unit) <> '' AND
                weighing_source IS NOT NULL)
        ),
...
CREATE INDEX idx_receipt_weighing_record_item_recorded
    ON receipt_weighing_record (purchase_receipt_item_id, recorded_at DESC) WHERE deleted = FALSE;
...
(
    ...
    CONSTRAINT ck_inventory_movement_type CHECK (movement_type IN (
                                                                   'PURCHASE_IN', 'SALES_OUT', 'RETURN_IN',
                                                                   'PURCHASE_RETURN_OUT',
...
CREATE UNIQUE INDEX uk_inventory_movement_receipt_item
    ON inventory_movement (source_document_type, source_document_item_id) WHERE deleted = FALSE AND source_document_type = 'PURCHASE_RECEIPT' AND movement_type = 'PURCHASE_IN';
...
CREATE INDEX idx_inventory_movement_source
    ON inventory_movement (source_document_type, source_document_id, source_document_item_id) WHERE deleted = FALSE;
