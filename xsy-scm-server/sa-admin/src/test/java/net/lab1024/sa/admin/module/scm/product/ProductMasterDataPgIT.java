...
class ProductMasterDataPgIT {
    @Autowired ProductSpuService service;
    @Autowired ProductQueryService query;
    @Autowired ProductBatchService batch;
    @Autowired ProductUomService uom;
    @Autowired ProductTagService tags;
    @Autowired ProductCategoryService categories;
    @Autowired ProductSkuOptionQueryService optionService;
    @Autowired ProductSkuOptionDao optionDao;
    ...
    @BeforeEach void operator() {
        ...
        var employee=new RequestEmployee(); employee.setEmployeeId(1L); employee.setActualName("PCO-1 IT"); employee.setUserType(UserTypeEnum.ADMIN_EMPLOYEE);
        SmartRequestUtil.setRequestUser(employee);
    }
    @AfterEach void clearOperator() { SmartRequestUtil.remove(); }

    /** SKU 编码在整个库内唯一，所以每个商品都要换新编码。 */
    private String code() { return prefix+"-"+(++sequence); }

    private ProductSpuAddForm product(String unit) {
        var form=new ProductSpuAddForm(); var code=code();
        form.setSpuCode(code); form.setName(code+"商品");
        ...
        sku.setSpecName("大"); sku.setSaleUnit(unit); sku.setProductType("NON_STANDARD"); sku.setStatus("ON_SHELF");
        form.setSkuList(new ArrayList<>(List.of(sku)));
        return form;
    }
    /** 详情页原样回填，用来验证「改一个无关字段」会不会被新增校验误伤。 */
    private ProductSpuUpdateForm edit(Long id) {
        var detail=query.detail(id); var form=new ProductSpuUpdateForm(); BeanUtils.copyProperties(detail,form);
        form.setSkuList(detail.getSkuList().stream().map(vo -> { var sku=new ProductSkuForm(); BeanUtils.copyProperties(vo,sku); return sku; }).toList());
        form.setImages(detail.getImages().stream().map(vo -> { var img=new ProductImageForm(); BeanUtils.copyProperties(vo,img); return img; }).toList());
        form.setTagIds(detail.getTags().stream().map(ProductSpuTagVO::getTagId).toList());
        return form;
    }
    private Long skuId(Long spuId) { return jdbc.queryForObject("SELECT id FROM product_sku WHERE spu_id=? AND deleted=FALSE",Long.class,spuId); }
    private Integer version(Long spuId) { return query.detail(spuId).getVersion(); }
    private ProductDeleteForm deletion(Long spuId) { var form=new ProductDeleteForm(); form.setSpuId(spuId); form.setVersion(version(spuId)); return form; }
    ...
    private Long tag(String suffix) {
        var form=new ProductTagAddForm(); form.setTagCode(prefix+suffix); form.setName(prefix+suffix); form.setStatus("ENABLED"); return tags.add(form);
    }
    private ProductTagVO tagRow(Long id) {
        var query=new ProductAssistantQueryForm(); query.setKeyword(prefix);
    ...
    private ProductUomKeyForm uomKey(ProductUomVO vo) { var form=new ProductUomKeyForm(); form.setUomId(vo.getUomId()); form.setVersion(vo.getVersion()); return form; }
    private ProductTagKeyForm tagKey(ProductTagVO vo) { var form=new ProductTagKeyForm(); form.setTagId(vo.getTagId()); form.setVersion(vo.getVersion()); return form; }
    private void disableTag(Long id) {
        var form=new ProductTagUpdateForm(); BeanUtils.copyProperties(tagRow(id),form); form.setStatus("DISABLED"); tags.update(form);
    }
    private ProductBatchItemForm item(Long spuId,Integer version) { var item=new ProductBatchItemForm(); item.setSpuId(spuId); item.setVersion(version); return item; }
    private ProductSpuQueryForm search() { var form=new ProductSpuQueryForm(); form.setPageNum(1L); form.setPageSize(50L); form.setKeyword(prefix); return form; }
    private List<Long> hits(ProductSpuQueryForm form) { return query.query(form).getList().stream().map(ProductSpuVO::getSpuId).toList(); }
    private List<String> selectableSkuCodes() {
        var form=new ProductSkuOptionQueryForm(); form.setKeyword(prefix); form.setLimit(50);
    ...
    void uomIsUniqueByCodeAndNameAndRetiredOnceReferenced() {
        ...
        var add=new ProductUomAddForm(); add.setUomCode("UOM-"+prefix); add.setName(prefix+"单位"); add.setCategory("WEIGHT");
        add.setPrecisionScale(2); add.setStatus("ENABLED"); add.setSortOrder(900);
        Long id=uom.add(add);
        assertThat(uomRow(id).getReferencedCount()).isZero();
        var sameName=new ProductUomAddForm(); BeanUtils.copyProperties(add,sameName); sameName.setUomCode("UOM-"+prefix+"X");
        conflict(() -> uom.add(sameName),UOM_NAME_DUPLICATE);
        var sameCode=new ProductUomAddForm(); BeanUtils.copyProperties(add,sameCode); sameCode.setName(prefix+"另一单位");
        ...
        var stale=new ProductUomUpdateForm(); BeanUtils.copyProperties(uomRow(id),stale); stale.setVersion(9);
        conflict(() -> uom.update(stale),VERSION_CONFLICT);

        var form=product(prefix+"单位"); Long spu=service.add(form);
        ...
        var retire=new ProductUomUpdateForm(); BeanUtils.copyProperties(uomRow(id),retire); retire.setStatus("DISABLED"); uom.update(retire);
    ...
    private ProductUomKeyForm uomKey(Long id,Integer version) { var form=new ProductUomKeyForm(); form.setUomId(id); form.setVersion(version); return form; }
    ...
    void dictionaryOnlyGuardsUnitsWrittenFromNowOn() {
        ...
        var unknown=edit(spu); unknown.getSkuList().getFirst().setSaleUnit("不存在单位");
        conflict(() -> service.update(unknown),UOM_NOT_USABLE);
        var known=edit(spu); known.getSkuList().getFirst().setSaleUnit("箱"); service.update(known);
        assertThat(query.detail(spu).getSkuList().getFirst().getSaleUnit()).isEqualTo("箱");
    }

    @Test
    void masterStatusOnlyNarrowsTheSelectableScope() {
        var form=product("kg"); form.setMasterStatus("DISABLED"); Long spu=service.add(form);
        ...
        var enable=edit(spu); enable.setMasterStatus("ENABLED"); service.update(enable);
        assertThat(selectableSkuCodes()).containsExactly(skuCode);
        var archived=product("kg"); archived.setMasterStatus("ARCHIVED"); Long archivedId=service.add(archived);
        assertThat(selectableSkuCodes()).containsExactly(skuCode);
    ...
    void archivedProductCannotStayOnShelfAtServiceAndDatabaseLevel() {
        var form=product("kg"); form.setStatus("ON_SHELF"); form.setMasterStatus("ARCHIVED");
        ...
        var archive=edit(rejected); archive.setStatus("ON_SHELF"); archive.setMasterStatus("ARCHIVED");
        conflict(() -> service.update(archive),MASTER_STATUS_SALE_CONFLICT);
        Long spu=service.add(product("kg"));
        var off=edit(spu); off.setMasterStatus("ARCHIVED"); service.update(off);
        var backOnline=new ProductStatusForm(); backOnline.setSpuId(spu); backOnline.setVersion(version(spu)); backOnline.setStatus("ON_SHELF");
    ...
    void tagStaysVisibleOnBoundProductsAndOnlyBlocksNewBindings() {
        var sameName=new ProductTagAddForm(); sameName.setTagCode(prefix+"DUP"); sameName.setName(prefix+"KEEP"); sameName.setStatus("ENABLED");
        Long bound=tag("KEEP");
        conflict(() -> tags.add(sameName),TAG_NAME_DUPLICATE);
        Long retire=tag("RETIRE"), off=tag("OFF");
        var form=product("kg"); form.setTagIds(List.of(bound,retire)); Long spu=service.add(form);
        assertThat(query.detail(spu).getTags()).extracting(ProductSpuTagVO::getTagId).containsExactlyInAnyOrder(bound,retire);
        conflict(() -> { var other=product("kg"); other.setTagIds(List.of(off,999999L)); service.add(other); },TAG_NOT_FOUND);
        conflict(() -> tags.delete(tagKey(tagRow(bound))),TAG_REFERENCED);

        disableTag(retire);
        var keepHistory=edit(spu); keepHistory.setBrandName("测试品牌"); service.update(keepHistory);
        ...
        disableTag(off);
        var addDisabled=edit(spu); addDisabled.setTagIds(List.of(bound,retire,off));
        conflict(() -> service.update(addDisabled),TAG_NOT_USABLE);
        var dropRetired=edit(spu); dropRetired.setTagIds(List.of(bound)); service.update(dropRetired);
        conflict(() -> { var rebind=edit(spu); rebind.setTagIds(List.of(bound,retire)); service.update(rebind); },TAG_NOT_USABLE);
    ...
    private ProductTagKeyForm tagKey(Long id,Integer version) { var form=new ProductTagKeyForm(); form.setTagId(id); form.setVersion(version); return form; }
    ...
    void batchRejectsEveryStaleRowBeforeWritingAnything() {
        ...
        archived.setStatus("ON_SHELF"); archived.setMasterStatus("ARCHIVED");
    ...
    void batchAppliesCategoryAndEveryTagModeToWholeSelection() {
        ...
        var category=new ProductSpuBatchCategoryForm(); category.setCategoryId(leaf);
    ...
    private ProductSpuBatchTagForm batchTags(String mode,List<Long> tagIds,Long... spuIds) {
        var form=new ProductSpuBatchTagForm(); form.setMode(mode); form.setTagIds(tagIds);
    ...
    void advancedFiltersCoverMnemonicTagMasterStatusStorageFlagsAndCreatedRange() {
        var form=product("kg"); form.setMnemonicCode(prefix+"ZJ"); form.setStorageMethod("CHILLED");
        Long tagged=tag("SEARCH"); form.setTagIds(List.of(tagged));
        Long spu=service.add(form);
        assertThat(hits(search())).containsExactly(spu);
        var byMnemonic=search(); byMnemonic.setKeyword(prefix+"zj"); assertThat(hits(byMnemonic)).containsExactly(spu);
        var byTag=search(); byTag.setTagIds(List.of(tagged)); assertThat(hits(byTag)).containsExactly(spu);
        var byMissingTag=search(); byMissingTag.setTagIds(List.of(tag("OTHER"))); assertThat(hits(byMissingTag)).isEmpty();
        var byMaster=search(); byMaster.setMasterStatus("DISABLED"); assertThat(hits(byMaster)).isEmpty();
        var byStorage=search(); byStorage.setStorageMethod("CHILLED"); assertThat(hits(byStorage)).containsExactly(spu);
        var otherStorage=search(); otherStorage.setStorageMethod("FROZEN"); assertThat(hits(otherStorage)).isEmpty();
        ...
        var recent=search(); recent.setCreatedFrom(OffsetDateTime.now().minusDays(1)); assertThat(hits(recent)).isEmpty();
        var window=search(); window.setCreatedFrom(OffsetDateTime.now().minusDays(30)); window.setCreatedTo(OffsetDateTime.now());
        assertThat(hits(window)).containsExactly(spu);
    }
    private ProductSpuQueryForm flag(boolean hasBarcode,boolean hasPrimaryImage) {
        var form=search(); form.setHasBarcode(hasBarcode); form.setHasPrimaryImage(hasPrimaryImage); return form;
    }
