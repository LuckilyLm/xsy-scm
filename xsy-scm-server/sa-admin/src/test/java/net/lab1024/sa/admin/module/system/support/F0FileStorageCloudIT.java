...
class F0FileStorageCloudIT {
    @Autowired IFileStorageService storage;
    @Autowired FileService files;
    @Autowired FileDao dao;
    @Autowired FileConfig config;
    ...
    @BeforeEach void context() {
        ...
        employee = new RequestEmployee(); employee.setEmployeeId(1L); employee.setActualName("F0 IT");
        employee.setUserType(UserTypeEnum.ADMIN_EMPLOYEE); employee.setAdministratorFlag(true);
    ...
    MockMultipartFile image() { return new MockMultipartFile("file", "f0-" + UUID.randomUUID() + ".png", "image/png", png); }
    ...
    @Test void privateUploadPersistsMetadataAndDownloadsOriginalBytes() throws Exception {
        var file = upload(1);
        ...
        assertThat(signed.statusCode()).isEqualTo(200); assertThat(signed.body()).isEqualTo(png);
    ...
    @Test void publicPolicyAllowsGetButNotAnonymousList() throws Exception {
        ...
        assertThat(result.getOk()).isTrue();
        var file = result.getData(); objects.add(file.getFileKey());
    ...
    @Test void signatureExpiresAndSubsequentReadGetsAFreshSignature() throws Exception {
        var file = upload(1);
    ...
    @Test void storageDeleteRemovesObjectButPreservesNativeMetadataLifecycle() throws Exception {
        var file = upload(1);
    ...
    @Test void validatesMimeLimitAndFolderBeforeCreatingRows() {
