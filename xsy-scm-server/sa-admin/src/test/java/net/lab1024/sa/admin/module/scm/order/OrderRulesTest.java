...
class OrderRulesTest {
    ...
    void stateGraph(String from,String to,boolean allowed){assertThat(OrderStateMachine.canTransition(from,to)).isEqualTo(allowed);}
    @Test void nullPropagatesButFreePricesAndHalfUpRemainValid(){
    ...
    @ParameterizedTest @ValueSource(strings={"1","1.00000","-1.0000","1e4","100000000000000.0000","0.0000"})
    void rejectsInvalidQuantity(String value){assertThatThrownBy(()->OrderValidator.decimal(value,true)).isInstanceOf(ScmBusinessException.class);}
    @Test void diffKeepsIdentityAndRejectsStaleOrForeignItems(){
        var old=new SalesOrderItemEntity();old.setId(7L);old.setVersion(2);
        var retained=new SalesOrderItemEntity();retained.setId(7L);retained.setVersion(2);
        var added=new SalesOrderItemEntity();
        var diff=SalesOrderItemChangeSet.between(List.of(old),List.of(retained,added));assertThat(diff.updated()).containsExactly(retained);assertThat(diff.inserted()).containsExactly(added);
        retained.setVersion(1);assertThatThrownBy(()->SalesOrderItemChangeSet.between(List.of(old),List.of(retained))).isInstanceOf(ScmBusinessException.class);
        retained.setId(8L);assertThatThrownBy(()->SalesOrderItemChangeSet.between(List.of(old),List.of(retained))).isInstanceOf(ScmBusinessException.class);
        assertThat(SalesOrderItemChangeSet.between(List.of(old),List.of()).removed()).containsExactly(old);
    }
    @Test void canonicalHashHandlesNestedKeysAndDecimals(){
    ...
    @Test void numberingExpandsNaturallyAndPriceEnumHasNoUnpriced(){
    ...
    @Test void draftValidationEnforcesSourceDuplicateAndOverrideRules(){
        var f=new SalesOrderAddForm();f.setOrderSource("ADMIN");var i=new SalesOrderItemForm();i.setSkuId(1L);i.setOrderedQuantity("1.0000");i.setManualPriceOverride(false);f.setItems(List.of(i));
        OrderValidator.draft(f);f.setItems(List.of(i,i));assertThatThrownBy(()->OrderValidator.draft(f)).isInstanceOf(ScmBusinessException.class);f.setItems(List.of(i));
        i.setManualPriceOverride(true);i.setUnitPrice("0.0000");assertThatThrownBy(()->OrderValidator.draft(f)).isInstanceOf(ScmBusinessException.class);i.setOverrideReason("赠送");OrderValidator.draft(f);
        f.setOriginalOrderId(9L);assertThatThrownBy(()->OrderValidator.draft(f)).isInstanceOf(ScmBusinessException.class);f.setOrderSource("SUPPLEMENT");f.setSupplementReason("补录");OrderValidator.draft(f);
    ...
    @Test void newCodesDoNotCollideWithExistingDomains(){
        var seen=new HashSet<Integer>();
        for(var c:net.lab1024.sa.admin.module.scm.pricing.constant.PricingErrorCode.values()) seen.add(c.getCode());
        for(var c:net.lab1024.sa.admin.module.scm.customer.constant.CustomerErrorCode.values()) seen.add(c.getCode());
