...
class ProductSkuChangeSetTest {
    @Test void preservesExistingIdentityAndSeparatesInsertUpdateAndRemove() {
        var old=new ProductSkuEntity(); old.setId(7L); old.setVersion(2);
        var removed=new ProductSkuEntity(); removed.setId(8L);
        var keep=new ProductSkuForm(); keep.setSkuId(7L); keep.setVersion(2);
        ...
        assertThat(keep.getSkuId()).isEqualTo(7L); assertThat(keep.getVersion()).isEqualTo(2);
    }
    @Test void rejectsForeignAndRepeatedIdentity() {
        var old=new ProductSkuEntity(); old.setId(7L);
        var foreign=new ProductSkuForm(); foreign.setSkuId(8L); foreign.setVersion(0);
