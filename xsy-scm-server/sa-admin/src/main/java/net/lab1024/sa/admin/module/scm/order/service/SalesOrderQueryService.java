...
public class SalesOrderQueryService {
    ...
    public PageResult<SalesOrderVO> query(SalesOrderQueryForm f) {
        var page=SmartPageUtil.convert2PageQuery(f);var rows=orders.query(page,f);
        return SmartPageUtil.convert2PageResult(page,rows.stream().map(this::order).toList());
    }
    public SalesOrderVO order(SalesOrderEntity row) {var v=new SalesOrderVO();BeanUtils.copyProperties(row,v);v.setOrderId(row.getId());return v;}
    public SalesOrderDetailVO detail(Long id) {
        var row=orders.selectById(id);if(row==null) throw new ScmBusinessException(ORDER_NOT_FOUND);
        var v=new SalesOrderDetailVO();BeanUtils.copyProperties(order(row),v);
        v.setItems(items.list(id).stream().sorted(Comparator.comparing(SalesOrderItemEntity::getSortOrder).thenComparing(SalesOrderItemEntity::getId)).map(x->{var i=new SalesOrderItemVO();BeanUtils.copyProperties(x,i);i.setItemId(x.getId());return i;}).toList());
        v.setAddress(addresses.list(id).stream().findFirst().map(x->{var a=new OrderAddressSnapshotVO();BeanUtils.copyProperties(x,a);return a;}).orElse(null));return v;
    }
    public PageResult<OrderOperationLogVO> logs(OrderLogQueryForm f) {
        var page=SmartPageUtil.convert2PageQuery(f);var rows=logs.query(page,f);
        ...
        return SmartPageUtil.convert2PageResult(page,rows.stream().map(x->{var v=new OrderOperationLogVO();BeanUtils.copyProperties(x,v);v.setLogId(x.getId());v.setOperatorName(names.getOrDefault(Long.valueOf(x.getOperator().split(":")[1]),x.getOperator()));return v;}).toList());
    }
}
