...
public class OrderIdempotencyService {
    ...
    public Claim claim(String scope,String key,Object request) {
        ...
        key=key.trim();if(key.length()>200) throw new ScmBusinessException(ORDER_IDEMPOTENCY_KEY_INVALID);
        ...
        var row=new IdempotencyRecordEntity();row.setOperationScope(scope);row.setIdempotencyKey(key);row.setRequestHash(hash);row.setCreatedBy(ScmOperator.current());
    ...
    public <T> T replay(Claim claim,Class<T> type) {return json.convertValue(claim.record().getResultData().get("value"),type);}
    public void complete(Claim claim,String type,Long id,Object result) {
        var row=claim.record();row.setResultId(id);row.setResultType(type);
        var value=new LinkedHashMap<String,Object>();value.put("value",json.convertValue(result,Object.class));row.setResultData(value);
        row.setUpdatedAt(java.time.OffsetDateTime.now());row.setUpdatedBy(ScmOperator.current());
