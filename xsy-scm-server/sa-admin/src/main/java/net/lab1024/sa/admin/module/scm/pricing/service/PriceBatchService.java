...
@Service @RequiredArgsConstructor public class PriceBatchService {
 private final PriceBatchWriter writer;private final PriceBatchAuditService audit;private final PriceBatchAuditDao audits;
 public PriceBatchResultVO submit(PriceBatchForm form) {
  if(form.getBatchKey()==null||form.getBatchKey().isBlank()||form.getBatchKey().length()>100||form.getRows()==null||form.getRows().isEmpty()||form.getRows().size()>500) throw new ScmBusinessException(PRICE_BATCH_ROW_INVALID);
  ...
  if(failures.isEmpty()) {
   try {return writer.write(form);}
   catch(PriceBatchWriter.Rejected e) {failures=e.failures;}
