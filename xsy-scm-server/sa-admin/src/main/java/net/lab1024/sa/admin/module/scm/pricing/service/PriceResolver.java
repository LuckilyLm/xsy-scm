...
public class PriceResolver {
    ...
    public List<ResolvedPriceVO> resolve(Long customerId, List<Long> ids, OffsetDateTime at) {
        ...
        return ids.stream().map(id -> {
            var row = new ResolvedPriceVO();
            row.setSkuId(id);
            var sku = byId.get(id);
            if (sku != null) {
                row.setSkuCode(sku.getSkuCode());
                row.setProductName(sku.getProductName());
                row.setSpecName(sku.getSpecName());
            }
            var reason = PriceValidation.unavailable(sku, all || visible.contains(id));
            row.setUnavailableReason(reason);
            row.setSellable(reason == null);
            // Q2: eligibility never short-circuits price lookup or erases a valid zero price.
            if (a.containsKey(id)) {
                var p = a.get(id);
                row.price(p.getUnitPrice(), ScmPriceSourceEnum.AGREEMENT, p.getId());
            } else if (t.containsKey(id)) {
                var p = t.get(id);
                row.price(p.getUnitPrice(), ScmPriceSourceEnum.CUSTOMER_TYPE, p.getId());
    ...
    public List<ResolvedPriceVO> requireResolvable(Long customerId, List<Long> ids, OffsetDateTime at) {
        ...
        if (rows.stream().anyMatch(r -> !r.isSellable() || r.getPriceStatus() == ScmPriceStatusEnum.UNPRICED))
