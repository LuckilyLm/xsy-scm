...
public class ProductCategoryService {
    ...
    public ProductCategoryVO detail(Long id) {
        var row = require(id);
        var vo = new ProductCategoryVO();
        BeanUtils.copyProperties(row, vo);
        vo.setCategoryId(id);
        vo.setCategoryPath(path(id, all()));
        return vo;
    }
    ...
    public void update(ProductCategoryUpdateForm form) {
        ...
        if (level != entity.getLevel() && (descendants.size() > 1 || countProducts(entity.getId()) > 0))
            throw new ScmBusinessException(CATEGORY_PARENT_INVALID);
    ...
    private void apply(ProductCategoryEntity entity, ProductCategoryAddForm form, Long self) {
        int level = resolveLevel(form.getParentId(), self);
        BeanUtils.copyProperties(form, entity, "version");
        entity.setCategoryCode(ProductAggregateValidator.normalizeCode(form.getCategoryCode()));
