...
public class DeliveryVehicleService {
    ...
    public PageResult<DeliveryVehicleEntity> query(DeliveryQueryForm form) {
        ...
        if(form.getKeyword()!=null && !form.getKeyword().isBlank()) wrapper.like(DeliveryVehicleEntity::getVehicleNo,form.getKeyword());
        if(form.getStatus()!=null && !form.getStatus().isBlank()) wrapper.eq(DeliveryVehicleEntity::getStatus,form.getStatus());
    ...
    public List<DeliveryVehicleEntity> options() {return dao.selectList(new LambdaQueryWrapper<DeliveryVehicleEntity>().eq(DeliveryVehicleEntity::getStatus,"ENABLED").orderByAsc(DeliveryVehicleEntity::getVehicleNo));}
