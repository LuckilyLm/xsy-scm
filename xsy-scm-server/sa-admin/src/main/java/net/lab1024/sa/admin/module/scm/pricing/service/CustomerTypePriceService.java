...
public class CustomerTypePriceService {
    ...
    public void delete(CustomerTypePriceDeleteForm form) {
        var e = require(form.getCustomerTypePriceId(), form.getVersion());
    ...
    private CustomerTypePriceEntity require(Long id, Integer version) {
        ...
        if (e == null) throw new ScmBusinessException(CUSTOMER_TYPE_PRICE_NOT_FOUND);
    ...
    private void validateAndLock(CustomerTypePriceAddForm form, Long exclude) {
        ...
        if (dao.lockParent(form.getCustomerTypeId()) == null) {
            types.requireSelectableType(form.getCustomerTypeId());
        }
        types.requireSelectableType(form.getCustomerTypeId());
        validation.requireSellable(form.getSkuId());
        if (dao.countOverlapping(form.getCustomerTypeId(), form.getSkuId(), form.getEffectiveFrom(), form.getEffectiveTo(), exclude) > 0)
    ...
    private void apply(CustomerTypePriceEntity e, CustomerTypePriceAddForm f) {
        e.setCustomerTypeId(f.getCustomerTypeId());
    ...
    private String snapshot(CustomerTypePriceEntity e) {
        ...
        fields.put("customerTypeId", e.getCustomerTypeId());
    ...
    private void log(CustomerTypePriceEntity e, String action, String before) {
