...
public class PriceBatchWriter {
    private final CustomerTypePriceDao prices;
    ...
    public PriceBatchResultVO write(PriceBatchForm form) {
        ...
        var typeMap = new HashMap<Long, CustomerTypeEntity>();
        types.selectByIds(rows.stream().map(PriceBatchRowForm::getCustomerTypeId).distinct().toList()).forEach(t -> typeMap.put(t.getId(), t));
        var skuMap = new HashMap<Long, ProductSkuOptionVO>();
        skus.selectByIds(rows.stream().map(PriceBatchRowForm::getSkuId).distinct().toList()).forEach(s -> skuMap.put(s.getSkuId(), s));
        ...
        for (var r : rows) {
            ...
            if (type == null || !"ENABLED".equals(type.getStatus()))
                failures.add(new PriceBatchRowFailureVO(r.getRowNumber(), r.getCustomerTypeId(), r.getSkuId(), 40431, "客户类型不存在或已停用"));
            if (PriceValidation.unavailable(skuMap.get(r.getSkuId()), true) != null)
                failures.add(new PriceBatchRowFailureVO(r.getRowNumber(), r.getCustomerTypeId(), r.getSkuId(), 40949, "SKU 不可售"));
            if (prices.countOverlapping(r.getCustomerTypeId(), r.getSkuId(), r.getEffectiveFrom(), r.getEffectiveTo(), null) > 0)
                failures.add(new PriceBatchRowFailureVO(r.getRowNumber(), r.getCustomerTypeId(), r.getSkuId(), 40935, "客户类型价有效期重叠"));
        }
        if (!failures.isEmpty()) throw new Rejected(failures);
        List<Long> ids = new ArrayList<>();
        for (var r : rows) {
            var f = new CustomerTypePriceAddForm();
            f.setCustomerTypeId(r.getCustomerTypeId());
            f.setSkuId(r.getSkuId());
            f.setUnitPrice(r.getUnitPrice());
            f.setEffectiveFrom(r.getEffectiveFrom());
            f.setEffectiveTo(r.getEffectiveTo());
            ids.add(service.add(f));
        }
