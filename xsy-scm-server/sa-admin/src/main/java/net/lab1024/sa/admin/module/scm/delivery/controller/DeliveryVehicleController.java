...
@RestController @RequestMapping("/scm/delivery/vehicles") @RequiredArgsConstructor
...
public class DeliveryVehicleController {
    ...
    @GetMapping @SaCheckPermission("scm:delivery:vehicle:query")
    public ResponseDTO<PageResult<DeliveryVehicleEntity>> query(@Valid @ModelAttribute DeliveryQueryForm form) {return ResponseDTO.ok(service.query(form));}
    @PostMapping @SaCheckPermission("scm:delivery:vehicle:edit") @OperateLog
    public ResponseDTO<Long> save(@Valid @RequestBody DeliveryVehicleForm form) {return ResponseDTO.ok(service.save(form));}
}
