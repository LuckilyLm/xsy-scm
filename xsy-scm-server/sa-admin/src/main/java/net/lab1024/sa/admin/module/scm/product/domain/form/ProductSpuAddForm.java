...
@Size(max=64) private String mnemonicCode;
@Size(max=100) private String brandName;
...
@DecimalMin("0") @DecimalMax("100") private BigDecimal lossRate;
@Min(0) @Max(365) private Integer purchaseWarningDays;
@Size(max=100) private String invoiceName;
@Size(max=32) private String taxCategoryCode;
@NotNull private Boolean taxExempt = false;
@DecimalMin("0") @DecimalMax("100") private BigDecimal taxRate;
/** 主档生命周期；null 表示新增时取 ENABLED、编辑时保持原值，上下架仍由 status 表达。 */
@Pattern(regexp="ENABLED|DISABLED|ARCHIVED") private String masterStatus;
@Size(max=50) @NotNull private List<@NotNull @Positive Long> tagIds = new ArrayList<>();
}
