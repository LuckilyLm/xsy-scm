...
VALUES (822,'确认入库',3,704,822,NULL,NULL,1,'scm:purchase:receipt:putaway','scm:purchase:receipt:putaway',NULL,NULL,true,1) ON CONFLICT(menu_id) DO NOTHING;
INSERT INTO t_menu(menu_id,menu_name,menu_type,parent_id,sort,path,component,perms_type,api_perms,web_perms,icon,context_menu_id,visible_flag,create_user_id)
VALUES (823,'启用',3,706,823,NULL,NULL,1,'scm:warehouse:enable','scm:warehouse:enable',NULL,NULL,true,1) ON CONFLICT(menu_id) DO NOTHING;
INSERT INTO t_menu(menu_id,menu_name,menu_type,parent_id,sort,path,component,perms_type,api_perms,web_perms,icon,context_menu_id,visible_flag,create_user_id)
VALUES (824,'停用',3,706,824,NULL,NULL,1,'scm:warehouse:disable','scm:warehouse:disable',NULL,NULL,true,1) ON CONFLICT(menu_id) DO NOTHING;

INSERT INTO t_role_menu(role_id,menu_id) SELECT 1,m.menu_id FROM t_menu m WHERE m.menu_id IN (822,823,824) AND NOT EXISTS(SELECT 1 FROM t_role_menu r WHERE r.role_id=1 AND r.menu_id=m.menu_id);
