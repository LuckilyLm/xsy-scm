...
ALTER TABLE purchase_operation_log
    DROP CONSTRAINT ck_purchase_operation_log_type;
ALTER TABLE purchase_operation_log
    ADD CONSTRAINT ck_purchase_operation_log_type CHECK (operation_type IN (
                                                                            'CREATE', 'UPDATE', 'SUBMIT', 'CANCEL',
                                                                            'SHORT_CLOSE', 'DELETE',
                                                                            'DEMAND_GENERATE', 'DEMAND_ALLOCATE',
                                                                            'RECEIPT_CREATE', 'RECEIPT_UPDATE',
                                                                            'RECEIPT_CONFIRM', 'RECEIPT_DELETE',
                                                                            'RECEIPT_PUTAWAY'));

ALTER TABLE purchase_operation_log
    DROP CONSTRAINT ck_purchase_operation_log_owner;
ALTER TABLE purchase_operation_log
    ADD CONSTRAINT ck_purchase_operation_log_owner CHECK (
        (operation_type = 'DEMAND_GENERATE'
            ...
            AND purchase_order_id IS NOT NULL AND purchase_receipt_id IS NULL)
            OR
        (operation_type IN ('RECEIPT_CREATE', 'RECEIPT_UPDATE', 'RECEIPT_CONFIRM', 'RECEIPT_DELETE', 'RECEIPT_PUTAWAY')
