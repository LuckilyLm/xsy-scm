...
INSERT INTO t_mail_template (template_code, template_subject, template_content, template_type, disable_flag, update_time, create_time) VALUES ('login_verification_code', '登录验证码', E'<!DOCTYPE HTML>\r\n<html>\r\n<head>\r\n  <title>登录提醒</title>\r\n  <meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"/>\r\n  <style>\r\n      * {\r\n          font-family: SimSun;\r\n          /* 4号字体 */\r\n          font-size: 18px;\r\n          /* 22磅行间距 */\r\n          line-height: 29px;\r\n      }\r\n\r\n      .main_font_size {\r\n          font-size: 12.0pt;\r\n      }\r\n\r\n      .mainContent {\r\n          line-height: 28px;\r\n      }\r\n\r\n      p {\r\n          margin: 0 auto;\r\n          text-align: justify;\r\n      }\r\n  </style>\r\n\r\n</head>\r\n<body>\r\n<div>\r\n  <div style=\"margin: 0px auto;width: 690px;\">\r\n    <div class=\"mainContent\">\r\n      <h1>验证码</h1>\r\n      <p>请在验证页面输入此验证码</p>\r\n      <p><b>${code}</b></p>\r\n      <p>验证码将于此电子邮件发出 5 分钟后过期。</p>\r\n      <p>如果你未曾提出此请求，可以忽略这封电子邮件。</p>\r\n    </div>\r\n\r\n  </div>\r\n</div>\r\n</body>\r\n</html>', 'freemarker', FALSE, '2024-08-06 09:13:08', '2024-07-28 13:56:06');

-- t_smart_job 种子数据
INSERT INTO t_smart_job (job_id, job_name, job_class, trigger_type, trigger_value, enabled_flag, param, last_execute_time, last_execute_log_id, sort, remark, deleted_flag, update_name, create_time, update_time) VALUES (1, '示例任务1', 'net.lab1024.sa.base.module.support.job.sample.SmartJobSample1', 'cron', '10 15 0/1 * * *', TRUE, '执行示例任务1', '2025-01-05 19:15:10', 7988, 1, '执行示例任务1', FALSE, '管理员', '2024-06-17 20:00:46', '2025-01-08 20:07:51');
INSERT INTO t_smart_job (job_id, job_name, job_class, trigger_type, trigger_value, enabled_flag, param, last_execute_time, last_execute_log_id, sort, remark, deleted_flag, update_name, create_time, update_time) VALUES (2, '示例任务2', 'net.lab1024.sa.base.module.support.job.sample.SmartJobSample2', 'fixed_delay', '120', TRUE, '执行示例任务2', '2025-01-08 19:56:59', 8144, 2, '执行示例任务2', FALSE, '管理员', '2024-06-18 20:45:35', '2025-01-08 19:57:00');

-- ---------------------------------------------------------------------
-- 校正 identity 序列（(MAX+1, is_called=false) 形式，空表从 1 开始）
-- ---------------------------------------------------------------------
SELECT setval(pg_get_serial_sequence('t_table_column', 'table_column_id'), (SELECT COALESCE(MAX(table_column_id), 0) FROM t_table_column) + 1, false);
SELECT setval(pg_get_serial_sequence('t_smart_job', 'job_id'), (SELECT COALESCE(MAX(job_id), 0) FROM t_smart_job) + 1, false);
SELECT setval(pg_get_serial_sequence('t_smart_job_log', 'log_id'), (SELECT COALESCE(MAX(log_id), 0) FROM t_smart_job_log) + 1, false);
SELECT setval(pg_get_serial_sequence('t_file', 'file_id'), (SELECT COALESCE(MAX(file_id), 0) FROM t_file) + 1, false);
SELECT setval(pg_get_serial_sequence('t_heart_beat_record', 'heart_beat_record_id'), (SELECT COALESCE(MAX(heart_beat_record_id), 0) FROM t_heart_beat_record) + 1, false);
SELECT setval(pg_get_serial_sequence('t_change_log', 'change_log_id'), (SELECT COALESCE(MAX(change_log_id), 0) FROM t_change_log) + 1, false);
SELECT setval(pg_get_serial_sequence('t_data_tracer', 'data_tracer_id'), (SELECT COALESCE(MAX(data_tracer_id), 0) FROM t_data_tracer) + 1, false);
SELECT setval(pg_get_serial_sequence('t_feedback', 'feedback_id'), (SELECT COALESCE(MAX(feedback_id), 0) FROM t_feedback) + 1, false);
SELECT setval(pg_get_serial_sequence('t_help_doc', 'help_doc_id'), (SELECT COALESCE(MAX(help_doc_id), 0) FROM t_help_doc) + 1, false);
SELECT setval(pg_get_serial_sequence('t_help_doc_catalog', 'help_doc_catalog_id'), (SELECT COALESCE(MAX(help_doc_catalog_id), 0) FROM t_help_doc_catalog) + 1, false);
SELECT setval(pg_get_serial_sequence('t_notice', 'notice_id'), (SELECT COALESCE(MAX(notice_id), 0) FROM t_notice) + 1, false);
SELECT setval(pg_get_serial_sequence('t_notice_type', 'notice_type_id'), (SELECT COALESCE(MAX(notice_type_id), 0) FROM t_notice_type) + 1, false);
SELECT setval(pg_get_serial_sequence('t_message', 'message_id'), (SELECT COALESCE(MAX(message_id), 0) FROM t_message) + 1, false);
SELECT setval(pg_get_serial_sequence('t_oa_bank', 'bank_id'), (SELECT COALESCE(MAX(bank_id), 0) FROM t_oa_bank) + 1, false);
SELECT setval(pg_get_serial_sequence('t_oa_enterprise', 'enterprise_id'), (SELECT COALESCE(MAX(enterprise_id), 0) FROM t_oa_enterprise) + 1, false);
SELECT setval(pg_get_serial_sequence('t_oa_enterprise_employee', 'enterprise_employee_id'), (SELECT COALESCE(MAX(enterprise_employee_id), 0) FROM t_oa_enterprise_employee) + 1, false);
SELECT setval(pg_get_serial_sequence('t_oa_invoice', 'invoice_id'), (SELECT COALESCE(MAX(invoice_id), 0) FROM t_oa_invoice) + 1, false);
