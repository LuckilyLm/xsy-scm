"""Create and remove isolated W5 browser test accounts.

Credentials are accepted only through the process environment and are never
printed.  The fixture grants the W1/W2/W4 read+write capabilities needed to
build a confirmed sales order (the only W5 demand source), plus the whole W5
purchase permission set from V16 (menus 701-706 / 711-713 / 721-727 /
731-735 / 741 / 751-753).
"""
...
name = os.environ["W5_E2E_NAME"]
password = os.environ["W5_E2E_PASSWORD"]
if not re.fullmatch(r"w5_e2e_[a-z0-9_]{1,15}", name):
    raise ValueError("Unexpected fixture account name")
accounts = [name, name + "_read"]
quoted = ",".join("'" + value + "'" for value in accounts)

# W1-W4（造客户 / 商品 / 供应商 SKU / 已确认销售订单的前置权限）+ W5 全部采购权限。
full = ",".join(str(value) for value in [
    ...
    601, 602, 603, 604, 605, 611, 612, 613, 614, 615, 616, 617, 618, 619,
    621, 622, 623, 624, 625, 631, 632, 641,
    701, 702, 703, 704, 705, 706, 711, 712, 713,
    721, 722, 723, 724, 725, 726, 727, 731, 732, 733, 734, 735, 741, 751, 752, 753,
])

# 只读账号：只给查询类菜单，用来验证「无权限时写命令被拒」（30005）。
read = ",".join(str(value) for value in [
    ...
    701, 702, 703, 704, 705, 706, 711, 721, 731, 741, 751,
])
...
if sys.argv[1] == "setup":
    ...
    for account, menus in [(accounts[0], full), (accounts[1], read)]:
        ...
        statements.extend([
            f"INSERT INTO t_employee(employee_uid,login_name,login_pwd,actual_name,department_id) SELECT '{account}','{account}','{hashed}','W5 E2E',min(department_id) FROM t_department;",
            f"INSERT INTO t_role(role_name,role_code) VALUES ('W5 E2E','{account}');",
...
print("W5 E2E fixture " + sys.argv[1] + " complete")
