"""Create and remove isolated W3 browser test accounts.
...
and customer data, plus the W3 pricing permissions.
"""
...
name = os.environ["W3_E2E_NAME"]
password = os.environ["W3_E2E_PASSWORD"]
if not re.fullmatch(r"w3_e2e_[a-z0-9_]{1,15}", name):
...
if sys.argv[1] == "setup":
    ...
    for account, menus in [(accounts[0], full), (accounts[1], read)]:
        ...
        statements.extend([
            f"INSERT INTO t_employee(employee_uid,login_name,login_pwd,actual_name,department_id) SELECT '{account}','{account}','{hashed}','W3 E2E',min(department_id) FROM t_department;",
            f"INSERT INTO t_role(role_name,role_code) VALUES ('W3 E2E','{account}');",
...
print("W3 E2E fixture " + sys.argv[1] + " complete")
