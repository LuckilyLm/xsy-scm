...
<template>
  <a-form class="smart-query-form">
    <a-row class="smart-query-form-row">
      <a-form-item label="产品" class="smart-query-form-item">
        <ProductSelect v-model:value="queryForm.productId" width="180px" />
      </a-form-item>
      <a-form-item label="供应商" class="smart-query-form-item">
        <SupplierSelect v-model:value="queryForm.supplierId" width="180px" />
      </a-form-item>
      <a-form-item class="smart-query-form-item">
        <a-button-group>
          <a-button type="primary" @click="onSearch" v-privilege="'product:supplier:query'">
            <template #icon><SearchOutlined /></template>
            查询
          </a-button>
          <a-button @click="resetQuery" v-privilege="'product:supplier:query'">
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        <a-button @click="addOrUpdate()" type="primary" v-privilege="'product:supplier:add'">
          <template #icon><PlusOutlined /></template>
          新建
        </a-button>
        <a-button @click="confirmBatchDelete" danger :disabled="selectedRowKeyList.length === 0" v-privilege="'product:supplier:batchDelete'">
          <template #icon><DeleteOutlined /></template>
          批量删除
        </a-button>
      </div>
      <div class="smart-table-setting-block">
        <TableOperator v-model="columns" :tableId="TABLE_ID_CONST.BUSINESS.PRODUCT.SUPPLIER" :refresh="queryData" />
      </div>
    </a-row>
    <a-table
      size="small"
      ...
      <template #bodyCell="{ text, record, column }">
        ...
        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate">
            <a-button @click="addOrUpdate(record)" type="link" v-privilege="'product:supplier:update'">编辑</a-button>
            <a-button @click="deleteOne(record)" danger type="link" v-privilege="'product:supplier:delete'">删除</a-button>
          </div>
        </template>
      </template>
    </a-table>
  ...
  <a-drawer :title="form.id ? '编辑' : '添加'" :width="450" :open="visible" :body-style="{ paddingBottom: '80px' }" @close="onClose">
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 7 }">
      ...
      <a-form-item label="供货价" name="supplyPrice">
        <a-input-number style="width: 100%" v-model:value="form.supplyPrice" :min="0" />
      </a-form-item>
...
<script setup lang="ts">
  ...
  import { productSupplierApi } from '/@/api/business/product/product-supplier-api';
  ...
  const columns = ref([
    { title: 'ID', dataIndex: 'id', resizable: true, width: 100 },
    { title: '产品ID', dataIndex: 'productId', resizable: true, width: 120 },
    { title: '供应商ID', dataIndex: 'supplierId', resizable: true, width: 120 },
    { title: '供货价', dataIndex: 'supplyPrice', resizable: true, width: 120 },
    { title: '是否默认', dataIndex: 'defaultFlag', resizable: true, width: 110 },
  ...
  async function queryData() {
    ...
    try {
      let res = await productSupplierApi.query(queryForm);
  ...
  function onSubmit() {
    formRef.value
      ...
      .then(async () => {
        ...
        try {
          if (form.id) {
            await productSupplierApi.update(form);
          } else {
            await productSupplierApi.add(form);
          }
  ...
  async function doDeleteOne(record) {
    try {
      SmartLoading.show();
      await productSupplierApi.delete(record.id);
  ...
  async function batchDelete() {
    try {
      SmartLoading.show();
      await productSupplierApi.batchDelete(selectedRowKeyList.value);
      message.success('删除成功');
      queryData();
    } catch (e) {
      smartSentry.captureError(e);
