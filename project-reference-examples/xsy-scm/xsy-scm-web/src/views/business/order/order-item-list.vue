...
<template>
  <a-form class="smart-query-form">
    <a-row class="smart-query-form-row">
      ...
      <a-form-item class="smart-query-form-item">
        <a-button-group>
          <a-button type="primary" @click="onSearch" v-privilege="'order:item:query'">
            <template #icon><SearchOutlined /></template>
            查询
          </a-button>
          <a-button @click="resetQuery" v-privilege="'order:item:query'">
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        <a-button @click="addOrUpdate()" type="primary" v-privilege="'order:item:add'">
          <template #icon><PlusOutlined /></template>
          新建
        </a-button>
        <a-button @click="confirmBatchDelete" danger :disabled="selectedRowKeyList.length === 0" v-privilege="'order:item:batchDelete'">
          <template #icon><DeleteOutlined /></template>
          批量删除
        </a-button>
      </div>
      <div class="smart-table-setting-block">
        <TableOperator v-model="columns" :tableId="TABLE_ID_CONST.BUSINESS.ORDER.ITEM" :refresh="queryData" />
      </div>
    </a-row>
    <a-table
      size="small"
    ...
    >
      <template #bodyCell="{ text, record, column }">
        ...
        <template v-if="column.dataIndex === 'status'">
          <span>{{ $smartEnumPlugin.getDescByValue('ORDER_ITEM_STATUS_ENUM', text) }}</span>
        </template>
        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate">
            <a-button @click="addOrUpdate(record)" type="link" v-privilege="'order:item:update'">编辑</a-button>
            <a-button @click="deleteOne(record)" danger type="link" v-privilege="'order:item:delete'">删除</a-button>
          </div>
        </template>
      </template>
    </a-table>
  ...
  <a-drawer :title="form.itemId ? '编辑' : '添加'" :width="500" :open="visible" :body-style="{ paddingBottom: '80px' }" @close="onClose">
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 7 }">
      ...
      <a-form-item label="下单数量" name="quantity">
        <a-input-number style="width: 100%" v-model:value="form.quantity" :min="0" />
      </a-form-item>
      <a-form-item label="价格快照" name="snapshotPrice">
        <a-input-number style="width: 100%" v-model:value="form.snapshotPrice" :min="0" />
      </a-form-item>
      ...
      <a-form-item label="明细金额" name="itemAmount">
        <a-input-number style="width: 100%" v-model:value="form.itemAmount" :min="0" />
      </a-form-item>
      <a-form-item label="状态" name="status">
        <SmartEnumSelect enum-name="ORDER_ITEM_STATUS_ENUM" v-model:value="form.status" width="100%" />
      </a-form-item>
    </a-form>
    <div
      :style="{
...
<script setup lang="ts">
  ...
  import { PRICE_TYPE_ENUM, ORDER_ITEM_STATUS_ENUM } from '/@/constants/business/order/order-const';
  ...
  const columns = ref([
    ...
    { title: '下单数量', dataIndex: 'quantity', resizable: true, width: 110 },
    { title: '价格快照', dataIndex: 'snapshotPrice', resizable: true, width: 110 },
    { title: '价格类型', dataIndex: 'priceType', resizable: true, width: 110 },
    { title: '实际重量', dataIndex: 'actualWeight', resizable: true, width: 110 },
    { title: '实际数量', dataIndex: 'actualQuantity', resizable: true, width: 110 },
    { title: '明细金额', dataIndex: 'itemAmount', resizable: true, width: 110 },
  ...
  async function queryData() {
    ...
    try {
      let res = await orderItemApi.query(queryForm);
  ...
  function onSubmit() {
    formRef.value
      ...
      .then(async () => {
        ...
        try {
          if (form.itemId) {
            await orderItemApi.update(form);
          } else {
            await orderItemApi.add(form);
          }
  ...
  async function doDeleteOne(record) {
    try {
      SmartLoading.show();
      await orderItemApi.delete(record.itemId);
  ...
  async function batchDelete() {
    try {
      SmartLoading.show();
      await orderItemApi.batchDelete(selectedRowKeyList.value);
      message.success('删除成功');
      queryData();
    } catch (e) {
      smartSentry.captureError(e);
