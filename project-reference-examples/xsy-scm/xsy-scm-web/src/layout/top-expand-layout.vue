...
<template>
  <a-layout class="admin-layout" style="min-height: 100%">
    ...
    <a-layout class="admin-layout-main" :style="`height: ${windowHeight}px`" id="xsyScmMain">
      <!-- 顶部头部信息 -->
      ...
      <!--中间内容-->
      <a-layout-content class="admin-layout-content" id="xsyScmLayoutContent">
...
<script setup lang="ts">
  ...
  onMounted(() => {
    if (watermarkFlag.value) {
      watermark.set('xsyScmLayoutContent', useUserStore().actualName);
    } else {
      watermark.clear();
    }
  });
  ...
  //回到顶部
  const backTopTarget = () => {
    return document.getElementById('xsyScmMain');
  };
  // ----------------------- keep-alive相关 -----------------------
