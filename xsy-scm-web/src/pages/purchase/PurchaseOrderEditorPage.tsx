            ...
            <Space direction="vertical" size="large" style={{width: '100%'}}>
                ...
                >
                    <Form.Item name="supplierId" label="供应商" rules={[{required: true, message: '请选择供应商'}]}>
                        <Select
                            style={{width: 220}}
                            options={(suppliers.data ?? []).filter((x) => x.status === 'ENABLED').map((x) => ({
                                value: x.id,
                                label: x.name
                            }))}
                        />
                    ...
                    <Form.Item name="warehouseId" label="仓库" rules={[{required: true, message: '请选择仓库'}]}>
                        <Select
                            style={{width: 220}}
                            options={(warehouses.data ?? []).filter((x) => x.status === 'ENABLED').map((x) => ({
                                value: x.id,
                                label: x.name
                            }))}
                        />
                ...
                <Table
                    ...
                    columns={[
                        ...
                        {
                            title: '采购数量',
                            render: (_, row) => <Input value={row.quantity}
                                                       onChange={(event) => updateRow(row.demandId, {quantity: event.target.value})}/>,
                        },
                        {
                            title: '采购单价',
                            render: (_, row) => <Input value={row.price}
                                                       onChange={(event) => updateRow(row.demandId, {price: event.target.value})}/>,
                        },
                    ]}
                />
                <Space>
                    ...
                    <Permission authority={AUTHORITIES.purchaseManage}>
                        <Button type="primary" loading={submitting} disabled={loading || !rows.length}
                                onClick={submit}>保存采购单</Button>
                    </Permission>
                </Space>
            </Space>
        </PageContainer>
