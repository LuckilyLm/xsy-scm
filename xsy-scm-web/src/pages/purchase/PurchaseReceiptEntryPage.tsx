...
export function PurchaseReceiptEntryPage() {
  ...
  const canReceive = hasPermission(AUTHORITIES.purchaseManage);

  useEffect(() => {
    if (!canReceive || started.current || !Number.isSafeInteger(purchaseOrderId)) return;
  ...
  }, [canReceive, navigate, purchaseOrderId]);

  if (status === 'loading') {
    return <PageContainer><Spin /></PageContainer>;
  }
  if (!canReceive) {
    return <PageContainer><Alert type="error" showIcon message="当前账号没有收货权限" action={<Button onClick={() => navigate(`/purchases/orders/${purchaseOrderId}`)}>返回采购单</Button>} /></PageContainer>;
  }
