...
export function OrderListPage() {
  ...
  const columns: ProColumns<SalesOrder>[] = [
    ...
    { title:'操作',valueType:'option',width:130,fixed:'right',render:(_,r)=>[<Button key="detail" type="link" onClick={()=>navigate(`/orders/${r.id}`)}>详情</Button>,...(r.status==='DRAFT'?[<Permission key="edit" authority={AUTHORITIES.orderManage}><Button type="link" onClick={()=>navigate(`/orders/${r.id}/edit`)}>编辑</Button></Permission>]:[])] },
  ];
  return <PageContainer><div className={styles.toolbar}><Space><Input value={keyword} allowClear prefix={<SearchOutlined/>} placeholder="订单号 / 客户" onChange={e=>setKeyword(e.target.value)}/><Select allowClear value={status} placeholder="全部状态" options={['DRAFT','PENDING','CONFIRMED','CANCELLED'].map(value=>({value,label:{DRAFT:'草稿',PENDING:'待审核',CONFIRMED:'已确认',CANCELLED:'已取消'}[value]}))} onChange={setStatus}/><Button type="primary" onClick={()=>actionRef.current?.reload()}>查询</Button></Space><Permission authority={AUTHORITIES.orderManage}><Space><Button onClick={()=>navigate('/orders/new?source=SUPPLEMENT')}>新建补单</Button><Button type="primary" icon={<PlusOutlined/>} onClick={()=>navigate('/orders/new')}>新建订单</Button></Space></Permission></div>{error?<Alert className={styles.error} type="error" showIcon message="订单加载失败" action={<Button onClick={()=>actionRef.current?.reload()}>重试</Button>}/>:null}<ProTable actionRef={actionRef} rowKey="id" search={false} options={false} columns={columns} scroll={{x:1150}} pagination={{defaultPageSize:20,showSizeChanger:true,showTotal:t=>`共 ${t} 条`}} request={async p=>{try{const data=await fetchOrders({page:p.current??1,pageSize:p.pageSize??20,keyword:keyword.trim()||undefined,status});setError(false);return {data:data.records,total:data.total,success:true};}catch{setError(true);return {data:[],total:0,success:true};}}}/></PageContainer>;
}
