...
export function ReturnDetailPage() {
    ...
    function withReason(kind: 'reject' | 'cancel') {
        ...
        Modal.confirm({
            ...
            content: <Input.TextArea placeholder="请输入原因" onChange={(event) => {
    ...
    if (query.isError || !data) return <PageContainer><Alert type="error" message="退货详情加载失败" action={<Button
        onClick={() => query.refetch()}>重试</Button>}/></PageContainer>;

    return <PageContainer>
        <div className={styles.header}>
            <Space><Button icon={<ArrowLeftOutlined/>}
                           onClick={() => navigate('/order-returns')}>返回列表</Button><Typography.Title level={4}
                                                                                                         className={styles.title}>{data.returnNo}</Typography.Title><StatusTag
                status={data.status}/></Space>
            {data.status === 'PENDING' ?
                <Permission authority={AUTHORITIES.orderManage}><Space><Button danger disabled={pending}
                                                                               onClick={() => withReason('cancel')}>取消申请</Button><Button
                    danger disabled={pending} onClick={() => withReason('reject')}>驳回</Button><Button type="primary"
                                                                                                        loading={pending}
                                                                                                        disabled={pending}
                                                                                                        onClick={approve}>批准退货</Button></Space></Permission> : null}
        </div>
        {error ? <Alert className={styles.error} type="error" message={error} showIcon
                        action={<Button onClick={() => query.refetch()}>重新加载</Button>}/> : null}
        <Descriptions bordered size="small" items={[
            {
                key: 'order',
                label: '原订单 ID',
                children: <Button type="link"
                                  onClick={() => navigate(`/orders/${data.orderId}`)}>{data.orderId}</Button>
            },
        ...
        <Table style={{marginTop: 16}} pagination={false} rowKey="id" dataSource={data.items} columns={[
            ...
            {title: '申请数量', dataIndex: 'requestedQuantity', align: 'right'},
            {
                ...
                align: 'right',
                render: (_, row: ReturnItem) => data.status === 'PENDING'
                    ? <Permission authority={AUTHORITIES.orderManage}
                                  fallback={<span>{row.approvedQuantity ?? '--'}</span>}>
                        <Input aria-label={`订单行 ${row.orderItemId} 批准数量`} inputMode="decimal"
                               value={approvalDraft.find((item) => item.returnItemId === row.id)?.approvedQuantity ?? ''}
                               onChange={(event) => setApprovalDraft((current) => updateApprovalQuantity(current, row.id, event.target.value))}/>
                    </Permission>
