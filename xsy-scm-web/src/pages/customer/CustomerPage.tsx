...
export function CustomerPage() {
  ...
  const columns: ProColumns<CustomerSummary>[] = [
    ...
    { title: '操作', valueType: 'option', fixed: 'right', width: 210, render: (_, r) => [
      <Permission key="edit" authority={AUTHORITIES.customerManage}>
        <Button type="link" size="small" onClick={() => { setDrawerId(r.id); setOpen(true); }}>编辑</Button>
      </Permission>,
      <Permission key="status" authority={AUTHORITIES.customerManage}>
        <Popconfirm title={`确认${r.status === 'ENABLED' ? '停用' : '启用'}该客户？`} onConfirm={() => mutate(r.id, () => updateCustomerStatus(r.id, r.version, r.status === 'ENABLED' ? 'DISABLED' : 'ENABLED'))}><Button type="link" size="small" loading={pendingId === r.id}>{r.status === 'ENABLED' ? '停用' : '启用'}</Button></Popconfirm>
      </Permission>,
      <Permission key="delete" authority={AUTHORITIES.customerManage}>
        <Popconfirm title="确认删除该客户？" okText="删除" okButtonProps={{ danger: true }} onConfirm={() => mutate(r.id, () => deleteCustomer(r.id, r.version))}><Button danger type="link" size="small" loading={pendingId === r.id}>删除</Button></Popconfirm>
      </Permission>,
    ] },
  ];
  return <PageContainer>
    ...
    <div className={styles.toolbar}><Permission authority={AUTHORITIES.customerManage}><Button type="primary" icon={<PlusOutlined />} onClick={() => { setDrawerId(null); setOpen(true); }}>新增客户</Button></Permission></div>
