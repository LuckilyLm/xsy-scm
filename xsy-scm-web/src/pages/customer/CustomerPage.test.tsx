...
vi.mock('../../api/customers', () => ({
    ...
    fetchCustomerTypes: vi.fn(),
    deleteCustomer: vi.fn(),
    updateCustomerStatus: vi.fn(),
    fetchCustomer: vi.fn(),
    createCustomer: vi.fn(),
    updateCustomer: vi.fn(),
    fetchSkuCatalog: vi.fn()
}));
vi.mock('../../api/auth', () => ({
...
function renderPage() {
    return renderWithProviders(<CustomerPage/>, {permissions: ALL_BUSINESS_PERMISSIONS});
}

beforeEach(() => {
    vi.mocked(fetchCustomerTypes).mockResolvedValue([]);
...
it('loads customers and maps keyword filter', async () => {
    renderPage();
    expect(await screen.findByText('春风食堂')).toBeInTheDocument();
    fireEvent.change(screen.getByPlaceholderText('客户名称 / 客户编码'), {target: {value: '春风'}});
    fireEvent.click(screen.getByRole('button', {name: /查\s*询/}));
    await waitFor(() => expect(fetchCustomers).toHaveBeenLastCalledWith(expect.objectContaining({
        keyword: '春风',
        page: 1,
        pageSize: 20
    })));
...
it('shows retry when loading fails', async () => {
    vi.mocked(fetchCustomers).mockRejectedValueOnce(new Error('offline'));
    renderPage();
    expect(await screen.findByText('客户数据加载失败')).toBeInTheDocument();
