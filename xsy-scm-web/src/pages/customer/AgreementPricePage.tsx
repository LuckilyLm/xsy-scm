...
export function AgreementPricePage() {
  ...
  const columns: ProColumns<AgreementPrice>[] = [
    ...
    { title: '操作', valueType: 'option', fixed: 'right', width: 140, render: (_, item) => [<Button key="edit" size="small" type="link" onClick={() => { setRecord(item); setOpen(true); }}>编辑</Button>, <Popconfirm key="delete" title="确认删除该协议价？" okText="删除" okButtonProps={{ danger: true }} onConfirm={() => remove(item)}><Button danger size="small" type="link" loading={pendingId === item.id}>删除</Button></Popconfirm>] },
  ];
  return <PageContainer>
    ...
    <div className={styles.toolbar}><Button type="primary" icon={<PlusOutlined />} onClick={() => { setRecord(null); setOpen(true); }}>新增协议价</Button></div>
