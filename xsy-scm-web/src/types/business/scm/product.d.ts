...
export interface ProductTagRef {
  tagId: ProductId; tagCode: string; name: string; status: AssistantStatus;
}
/** 主档扩展字段：保质期/损耗/税务只在详情 VO 出现，列表 VO 只带前置的常用字段。 */
interface ProductMasterFields {
  mnemonicCode?: string | null; brandName?: string | null; origin?: string | null;
  storageMethod?: StorageMethod | null; shelfLifeDays?: number | null; lossRate?: number | null;
  purchaseWarningDays?: number | null; invoiceName?: string | null; taxCategoryCode?: string | null;
  taxExempt?: boolean; taxRate?: number | null;
}
...
export interface ProductRow extends ProductMasterFields {
  spuId: ProductId; version: number; spuCode: string; name: string; alias?: string | null;
  categoryId?: ProductId; description?: string | null; status: ShelfStatus;
  categoryPath: string; categoryName: string; skuCount: number; masterStatus: MasterStatus;
  tags: ProductTagRef[]; skuList: ProductSku[]; images?: ProductImage[];
...
export interface ProductQuery {
  ...
  masterStatus?: MasterStatus; storageMethod?: StorageMethod; tagIds?: ProductId[];
  hasPrimaryImage?: boolean; hasBarcode?: boolean; createdFrom?: string | null; createdTo?: string | null;
  sortItemList?: { column: string; isAsc: boolean }[];
}
/** 辅助资料列表条件：字典规模有限，后端不启用分页。 */
export interface AssistantQuery { keyword?: string; status?: AssistantStatus }
export interface ProductUom {
  uomId: ProductId; version: number; uomCode: string; name: string; category: UomCategory;
  precisionScale: number; status: AssistantStatus; sortOrder: number; referencedCount: number;
}
export interface ProductUomAddForm {
  uomCode: string; name: string; category: UomCategory; precisionScale: number; status: AssistantStatus; sortOrder: number;
}
/** 编辑表单不含编码与名称：单位按名称字符串记账，改名会让历史数据失去真值来源，只能停用旧单位再新建。 */
export interface ProductUomUpdateForm {
  uomId: ProductId; version: number; category: UomCategory; precisionScale: number; status: AssistantStatus; sortOrder: number;
}
export interface ProductTag {
  tagId: ProductId; version: number; tagCode: string; name: string; status: AssistantStatus;
  sortOrder: number; productCount: number;
}
export interface ProductTagForm {
  tagId?: ProductId; version?: number; tagCode: string; name: string; status: AssistantStatus; sortOrder: number;
}
/** 批量命令逐行带乐观锁版本，冲突时后端能指到具体商品。 */
export interface ProductBatchItem { spuId: ProductId; version: number }
export interface ProductBatchResult {
  updatedCount: number; failedCount: number;
