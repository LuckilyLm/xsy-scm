...
function call<T>(method: string, path: string, data?: unknown): Promise<ScmResponse<T>> {
  return request({ url: `/scm/delivery${path}`, method, ...(method === 'get' ? { params: data } : { data }) }) as unknown as Promise<ScmResponse<T>>;
}
export const deliveryApi = {
  ...
  cancel: (id: Id, version: number, reason: string) => call<string>('post', `/routes/${id}/cancel`, { version, reason }),
  candidates: (query: Query) => call<ScmPage<CandidateOrder>>('get', '/candidate-orders', query),
  addOrders: (id: Id, version: number, orderIds: Id[], reason: string) => call<string>('post', `/routes/${id}/orders`, { version, orderIds, reason }),
  removeOrder: (id: Id, orderId: Id, version: number, reason: string) =>
    call<string>('delete', `/routes/${id}/orders/${orderId}`, { version, reason }),
  reorder: (id: Id, version: number, stopIds: Id[]) => call<string>('put', `/routes/${id}/stops/reorder`, { version, stopIds }),
  locate: (id: Id, stopId: Id, form: ScmLocation & { version: number; plannedArrivalTime?: string | null; remark?: string | null }) =>
