...
import type { CustomerDetail, CustomerPage, CustomerPayload, SalesPageParams, CustomerType, AgreementPrice, AgreementPricePayload } from '../types/sales';
...
export async function updateCustomerStatus(id: number, version: number, status: 'ENABLED' | 'DISABLED') { await apiClient.put(`/customers/${id}/status`, { version, status }); }
export async function deleteCustomer(id: number, version: number) { await apiClient.delete(`/customers/${id}`, { params: { version } }); }
export async function fetchAgreementPrices(params: SalesPageParams) { return (await apiClient.get<PageData<AgreementPrice>>('/customer-agreement-prices', { params })).data; }
...
export async function deleteAgreementPrice(id: number, version: number) { await apiClient.delete(`/customer-agreement-prices/${id}`, { params: { version } }); }
export async function fetchSkuCatalog(params: ProductPageParams) { return (await apiClient.get<PageData<ProductSummary>>('/products', { params })).data; }
