 ...
 * （销售出库 / 盘点 / 调拨都会写同一张流水表），把它的枚举挂到采购常量里，
 ...
 * 库存流水类型。
 *
 * **方向编码在类型里**：`PURCHASE_IN` / `STOCKTAKE_GAIN` 即「入」，
 * `SALES_OUT` / `STOCKTAKE_LOSS` 即「出」，因此流水没有独立的 `direction` 列，
 ...
 * 人类可读的来源单号用 `receiptNo`（收货单）或 `sourceDocumentNo`（出库单 / 盘点单）。
 */
