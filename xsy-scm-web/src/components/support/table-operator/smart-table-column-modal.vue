  ...
  <a-modal :width="800" :open="visible" title="设置列" :destroyOnClose="true" :closable="false">
    ...
    <div v-else>
      <a-alert type="info" show-icon class="smart-margin-bottom10">
        ...
        <template #message> 可以通过拖拽行直接修改顺序哦；（
          <pushpin-outlined/>
          为固定列，不可拖拽 ）
        </template>
      </a-alert>
          ...
          <template v-if="column.dataIndex === 'operate'">
            <div class="smart-table-operate" v-if="!record.fixed">
              <a-button @click="up(index)" v-show="index > 0" type="link" class="handle" size="small"
                        style="margin-right: 12px"> 上移
              </a-button>
              <a-button @click="down(index)" type="link" class="handle" size="small"
                        v-show="index !== tableData.length - 1"> 下移
              </a-button>
            </div>
          </template>
        </template>
    ...
    <template #footer>
      ...
      <a-button v-if="tableId" key="back" :loading="submitLoading" @click="reset" danger style="margin-left: 20px">
        恢复默认
      </a-button>
    </template>
  </a-modal>
