...
<template>
  <a-form class="smart-query-form" layout="inline" @finish="search">
    <a-form-item label="关键字" class="smart-query-form-item">
      <a-input v-model:value="query.keyword" placeholder="名称或编码" allow-clear/>
    </a-form-item>
    <a-form-item label="客户类型" class="smart-query-form-item">
      <CustomerTypeSelect v-model:value="query.customerTypeId" width="190px"/>
    </a-form-item>
    ...
    <a-form-item label="有效区间" class="smart-query-form-item">
      <a-range-picker v-model:value="range" show-time value-format="YYYY-MM-DDTHH:mm:ssZ" :allow-empty="[true,true]"/>
    </a-form-item>
    <a-form-item class="smart-query-form-item">
      <a-space>
        <a-button type="primary" html-type="submit" v-privilege="'scm:pricing:type-price:query'">查询</a-button>
  ...
  <a-card size="small" :bordered="false">
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        <a-button type="primary" v-privilege="'scm:pricing:type-price:add'" @click="drawer?.open()">新增客户类型价
        </a-button>
        <a-button v-privilege="'scm:pricing:type-price:batch'"
                  @click="router.push('/pricing/customer-type-price-batch')">批量调价
        </a-button>
      </div>
      <div class="smart-table-setting-block">
        <TableOperator v-model="columns" :table-id="TABLE_ID_CONST.BUSINESS.SCM_PRICING_TYPE_PRICE" :refresh="load"/>
      </div>
    </a-row>
    <a-table :data-source="rows" :columns="columns" row-key="customerTypePriceId" size="small" bordered
             :loading="loading" :pagination="false" :scroll="{x:1300}">
      <template #bodyCell="{record,column}">
        <template v-if="column.dataIndex==='unitPrice'"><span class="amount">{{ formatAmount(record.unitPrice) }}</span>
        </template>
        <template v-else-if="column.dataIndex==='effectiveTo'">{{ record.effectiveTo || '长期有效' }}</template>
        <template v-else-if="column.dataIndex==='action'">
          <div class="smart-table-operate">
            <a-button type="link" v-privilege="'scm:pricing:type-price:update'"
                      @click="drawer?.open(record.customerTypePriceId)">编辑
            </a-button>
            <a-button type="link" danger v-privilege="'scm:pricing:type-price:delete'" @click="remove(record)">删除
            </a-button>
          </div>
        </template>
      </template>
    ...
    <div class="smart-query-table-page">
      <a-pagination v-model:current="query.pageNum" v-model:page-size="query.pageSize" :total="total" show-size-changer
                    show-quick-jumper @change="load" :show-total="(n:number)=>`共 ${n} 条`"/>
    </div>
  </a-card>
