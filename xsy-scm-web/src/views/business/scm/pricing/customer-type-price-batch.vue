  ...
  <a-card title="客户类型价批量调价" size="small" :bordered="false">
    ...
    <a-form layout="inline" class="smart-query-form">
      <a-form-item label="批次号" required>
        <a-input v-model:value="batchKey" aria-label="批次号" :maxlength="100" style="width:320px"/>
      </a-form-item>
      <a-form-item>
        <a-button @click="add" :disabled="rows.length>=500||saving">新增行</a-button>
      </a-form-item>
      <a-form-item>
        <a-button type="primary" :loading="saving" v-privilege="'scm:pricing:type-price:batch'" @click="submit">
          提交整批
        </a-button>
      </a-form-item>
    </a-form>
    <a-alert v-if="error" :message="error" type="error" show-icon/>
    <a-table :data-source="rows" :columns="columns" row-key="rowNumber" :pagination="false" size="small" bordered
             :scroll="{x:1300}"
             :row-class-name="(r:BatchRow)=>failures.some(f=>f.rowNumber===r.rowNumber)?'failed-row':''">
      <template #bodyCell="{record,column}">
        <template v-if="column.dataIndex==='customerTypeId'">
          <CustomerTypeSelect v-model:value="record.customerTypeId" width="180px"/>
        </template>
        <template v-else-if="column.dataIndex==='skuId'">
          <SkuSelect v-model:value="record.skuId" width="260px"/>
        </template>
        <template v-else-if="column.dataIndex==='unitPrice'">
          <a-input v-model:value="record.unitPrice" aria-label="单价" inputmode="decimal"/>
        </template>
        <template v-else-if="column.dataIndex==='effectiveFrom'">
          <a-range-picker :value="[record.effectiveFrom,record.effectiveTo||'']" show-time
                          value-format="YYYY-MM-DDTHH:mm:ssZ" :allow-empty="[false,true]"
                          @change="(v:unknown)=>setPeriod(record,v)"/>
        </template>
        <template v-else-if="column.dataIndex==='error'"><span
            class="row-error">{{ failures.filter(f => f.rowNumber === record.rowNumber).map(f => f.message).join('；') }}</span>
        </template>
        <template v-else-if="column.dataIndex==='action'">
          <a-button type="link" danger :disabled="saving" @click="rows=rows.filter(r=>r.rowNumber!==record.rowNumber)">
            移除
          </a-button>
        </template>
      </template>
