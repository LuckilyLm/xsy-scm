...
<template>
  <a-form layout="inline" class="smart-query-form" @finish="search">
    <a-form-item label="来源">
      <a-select v-model:value="query.source" allow-clear style="width:150px"
                :options="[{value:'AGREEMENT',label:'客户协议价'},{value:'CUSTOMER_TYPE',label:'客户类型价'}]"/>
    </a-form-item>
    ...
    <a-form-item label="操作">
      <a-select v-model:value="query.operationType" allow-clear style="width:110px"
                :options="[{value:'CREATE',label:'新增'},{value:'UPDATE',label:'更新'},{value:'DELETE',label:'删除'}]"/>
    </a-form-item>
    <a-form-item label="有效区间">
      <a-range-picker v-model:value="effective" show-time value-format="YYYY-MM-DDTHH:mm:ssZ"
                      :allow-empty="[true,true]"/>
    </a-form-item>
    <a-form-item label="操作区间">
      <a-range-picker v-model:value="operated" show-time value-format="YYYY-MM-DDTHH:mm:ssZ"
                      :allow-empty="[true,true]"/>
    </a-form-item>
    <a-form-item>
      <a-button type="primary" html-type="submit" v-privilege="'scm:pricing:history:query'">查询</a-button>
    </a-form-item>
  </a-form>
  ...
  <a-card size="small" :bordered="false">
    ...
    <a-table :data-source="rows" :columns="columns" :row-key="(r:HistoryRow)=>`${r.source}-${r.historyId}`"
             :loading="loading" :pagination="false" size="small" bordered :scroll="{x:1550}">
      <template #bodyCell="{record,column}">
        <template v-if="column.dataIndex==='currentUnitPrice'">{{ formatAmount(record.currentUnitPrice) }}</template>
        <template v-else-if="column.dataIndex==='currentDeleted'">
          <a-tag v-if="record.currentDeleted" color="red">已删除</a-tag>
          <span v-else>有效记录</span></template>
        <template v-else-if="column.dataIndex==='action'">
          <a-button type="link" @click="selected=record">变更详情</a-button>
        </template>
