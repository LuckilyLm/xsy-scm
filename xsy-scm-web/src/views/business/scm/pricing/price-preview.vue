...
<template>
  <a-card title="取价试算" size="small" :bordered="false">
    <a-form layout="inline" class="smart-query-form" @finish="resolve">
      ...
      <a-form-item label="时点">
        <a-date-picker v-model:value="at" show-time value-format="YYYY-MM-DDTHH:mm:ssZ" placeholder="当前时点"/>
      </a-form-item>
      <a-form-item>
        <a-button type="primary" html-type="submit" :loading="loading" v-privilege="'scm:pricing:resolve:query'">试算
        </a-button>
      </a-form-item>
    </a-form>
    ...
    <a-table :columns="columns" :data-source="result?.items||[]" row-key="skuId" :loading="loading" :pagination="false"
      ...
      <template #bodyCell="{record,column}">
        ...
        <template v-else-if="column.dataIndex==='priceStatus'">
          <a-tag :color="record.priceStatus==='PRICED'?'green':'default'">
            {{ record.priceStatus === 'PRICED' ? '已定价' : '未定价' }}
          </a-tag>
        </template>
        <template v-else-if="column.dataIndex==='sellable'">
          <a-tag :color="record.sellable?'green':'red'">{{ record.sellable ? '可售' : '不可售' }}</a-tag>
        </template>
        <template v-else-if="column.dataIndex==='priceSource'">{{ sourceLabel(record.priceSource) }}</template>
        <template v-else-if="column.dataIndex==='unpricedReason'">{{ record.unpricedReason ? '无可用价格来源' : '—' }}
        </template>
        <template v-else-if="column.dataIndex==='unavailableReason'">{{ reasonLabel(record.unavailableReason) }}
        </template>
