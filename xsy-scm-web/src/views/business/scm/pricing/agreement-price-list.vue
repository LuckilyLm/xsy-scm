...
<template>
  <a-form class="smart-query-form" layout="inline" @finish="search">
    ...
    <a-form-item class="smart-query-form-item">
      <a-space>
        <a-button type="primary" html-type="submit" v-privilege="'scm:pricing:agreement:query'">查询</a-button>
  ...
  <a-card size="small" :bordered="false">
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        <a-button type="primary" v-privilege="'scm:pricing:agreement:add'" @click="drawer?.open()">新增客户协议价
        </a-button>
      </div>
      <div class="smart-table-setting-block">
        <TableOperator v-model="columns" :table-id="TABLE_ID_CONST.BUSINESS.SCM_PRICING_AGREEMENT" :refresh="load"/>
      </div>
    </a-row>
    <a-table :data-source="rows" :columns="columns" row-key="agreementPriceId" size="small" bordered :loading="loading"
      ...
      <template #bodyCell="{record,column}">
        ...
        <template v-else-if="column.dataIndex==='action'">
          <div class="smart-table-operate">
            <a-button type="link" v-privilege="'scm:pricing:agreement:update'"
                      @click="drawer?.open(record.agreementPriceId)">编辑
            </a-button>
            <a-button type="link" danger v-privilege="'scm:pricing:agreement:delete'" @click="remove(record)">删除
            </a-button>
          </div>
        </template>
      </template>
