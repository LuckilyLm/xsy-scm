...
const columns=ref<TableColumnsType<PriceRow>>([{title:'客户',dataIndex:'customerName',width:160},{title:'客户编码',dataIndex:'customerCode',width:130},{title:'SKU 编码',dataIndex:'skuCode',width:150},{title:'商品',dataIndex:'productName',width:160},{title:'规格',dataIndex:'specName',width:120},{title:'单价',dataIndex:'unitPrice',align:'right',width:120},{title:'生效时间',dataIndex:'effectiveFrom',width:200},{title:'结束时间',dataIndex:'effectiveTo',width:200},{title:'更新时间',dataIndex:'updatedAt',width:200},{title:'操作',dataIndex:'action',align:'right',fixed:'right',width:130}]);
...
function reset(){query.keyword=undefined;query.customerId=undefined;query.skuId=undefined;range.value=undefined;search();}
function remove(row:PriceRow){Modal.confirm({title:'删除这条客户协议价？',content:'删除后保留价格变更记录。',onOk:async()=>{try{await api.delete(row);await load();}catch(e){error.value=pricingError(e);throw e;}}});}
onMounted(load);
</script>
