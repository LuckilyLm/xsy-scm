<template>
  <a-modal v-model:open="open" title="导入商品" :width="720" :mask-closable="false"
    ...
    <a-alert type="info" show-icon class="hint"
             message="导入是整批事务：任意一行有错都不会写入任何商品。请先下载模板，按 SPU 编码组织多行 SKU。" />
    <a-space class="toolbar">
      ...
      <a-button type="primary" :loading="importing" :disabled="!file" @click="doImport">开始导入</a-button>
    </a-space>
    <p v-if="file" class="picked">已选文件：{{ file.name }}（{{ (file.size / 1024).toFixed(1) }} KB）</p>

    <a-result v-if="done && !hasErrors" status="success" :title="`成功导入 ${result?.importedProducts ?? 0} 个商品`"
...
import type { ProductImportResult } from '/@/types/business/scm/product';
...
const open = ref(false), file = ref<File | null>(null), importing = ref(false), templateLoading = ref(false),
    result = ref<ProductImportResult | null>(null), done = ref(false);
const errorColumns = [
...
async function doImport() {
  ...
  try {
    const response = await productApi.importProducts(file.value);
