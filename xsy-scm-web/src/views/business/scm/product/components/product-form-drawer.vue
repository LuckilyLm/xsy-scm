    ...
    <a-spin :spinning="loading">
      ...
      <a-form v-if="!loadFailed" ref="formRef" :model="form" layout="vertical">
        <a-row :gutter="20">
          ...
          <a-col :xs="24" :md="12"><a-form-item label="商品状态" name="status" help="只表达是否在售"><a-select v-model:value="form.status" :options="SHELF_STATUS_ENUM" /></a-form-item></a-col>
          <a-col :xs="24" :md="12"><a-form-item label="主档状态" name="masterStatus" help="停止引用后新订单与采购选不到该商品，已生成的单据不受影响"><a-select v-model:value="form.masterStatus" :options="MASTER_STATUS_ENUM" /></a-form-item></a-col>
        ...
        <a-row :gutter="20">
          <a-col :xs="24" :md="8"><a-form-item label="助记码" name="mnemonicCode" help="运营自维护的拼音缩写，用于列表模糊搜索；系统不自动生成"><a-input v-model:value="form.mnemonicCode" :maxlength="64" /></a-form-item></a-col>
          <a-col :xs="24" :md="8"><a-form-item label="品牌" name="brandName"><a-input v-model:value="form.brandName" :maxlength="100" /></a-form-item></a-col>
          <a-col :xs="24" :md="8"><a-form-item label="产地" name="origin"><a-input v-model:value="form.origin" :maxlength="100" /></a-form-item></a-col>
          <a-col :xs="24" :md="8"><a-form-item label="储存方式" name="storageMethod"><a-select v-model:value="form.storageMethod" :options="STORAGE_METHOD_ENUM" allow-clear placeholder="未维护" /></a-form-item></a-col>
          <a-col :xs="24" :md="8"><a-form-item label="保质期天数" name="shelfLifeDays"><a-input-number v-model:value="form.shelfLifeDays" :min="0" :max="36500" :precision="0" :controls="false" style="width: 100%" placeholder="未维护" /></a-form-item></a-col>
          <a-col :xs="24" :md="8"><a-form-item label="采购预警天数" name="purchaseWarningDays"><a-input-number v-model:value="form.purchaseWarningDays" :min="0" :max="365" :precision="0" :controls="false" style="width: 100%" placeholder="未维护" /></a-form-item></a-col>
          <a-col :xs="24" :md="8"><a-form-item label="损耗率（%）" name="lossRate" help="仅主数据参考，不参与任何金额计算"><a-input-number v-model:value="form.lossRate" :min="0" :max="100" :precision="4" :controls="false" style="width: 100%" /></a-form-item></a-col>
          <a-col :xs="24" :md="8"><a-form-item label="开票品名" name="invoiceName"><a-input v-model:value="form.invoiceName" :maxlength="100" /></a-form-item></a-col>
          <a-col :xs="24" :md="8"><a-form-item label="税收分类编码" name="taxCategoryCode"><a-input v-model:value="form.taxCategoryCode" :maxlength="32" /></a-form-item></a-col>
          <a-col :xs="24" :md="8"><a-form-item label="是否免税" name="taxExempt"><a-switch v-model:checked="form.taxExempt" /></a-form-item></a-col>
          <a-col :xs="24" :md="8"><a-form-item label="税率（%）" name="taxRate" help="财务模块未上线，不改变订单金额口径"><a-input-number v-model:value="form.taxRate" :min="0" :max="100" :precision="4" :controls="false" style="width: 100%" /></a-form-item></a-col>
          <a-col :span="24"><a-form-item label="商品标签" name="tagIds"><a-select v-model:value="form.tagIds" mode="multiple" :options="tagOptions" option-filter-prop="label" placeholder="选择标签" /></a-form-item></a-col>
        </a-row>
        ...
        <a-divider orientation="left">SKU 规格</a-divider><SkuEditor v-model="form.skuList" :units="units" />
      </a-form>
    </a-spin>
...
import type { ProductCategory, ProductForm, ProductId, ProductTag, ProductTagRef, ProductUom } from '/@/types/business/scm/product';
...
const tagOptions = computed(() => {
  ...
  return [...active, ...boundTags.value.filter((tag) => !known.has(String(tag.tagId))).map((tag) => ({ value: tag.tagId, label: `${tag.name}（已停用）` }))];
});
...
async function load(id?: ProductId) {
  ...
  try {
    ...
    categories.value = tree.data; units.value = uomOptions.data; tagChoices.value = tagOptionsResponse.data;
