...
<template>
  <a-form class="smart-query-form" layout="inline" @finish="search">
    <a-form-item label="关键字" class="smart-query-form-item">
      <a-input v-model:value="query.keyword" allow-clear placeholder="编码 / 名称" style="width: 200px"/>
    </a-form-item>
    <a-form-item label="状态" class="smart-query-form-item">
      <a-select v-model:value="query.status" allow-clear :options="ENABLE_STATUS_ENUM" style="width: 110px"/>
    </a-form-item>
  ...
  <a-table :data-source="rows" :columns="columns" row-key="uomId" :loading="loading" :pagination="false" size="small"
           bordered>
    <template #bodyCell="{ column, record }">
      ...
      <a-space v-else-if="column.dataIndex === 'action'" :size="0" class="smart-table-operate">
        ...
        <a-popconfirm v-else title="确认删除此计量单位？" @confirm="remove(record)">
          <a-button v-privilege="'scm:product:uom:delete'" type="link" danger size="small">删除</a-button>
        </a-popconfirm>
      </a-space>
    </template>
  </a-table>
...
const columns: TableColumnsType<ProductUom> = [
  ...
  {title: '小数位', dataIndex: 'precisionScale', width: 80, align: 'right'}, {
    title: '被引用',
