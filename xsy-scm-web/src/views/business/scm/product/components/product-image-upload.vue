<template>
  <div>
    ...
    <div class="images">
      <figure v-for="(image, index) in modelValue" :key="image.fileKey" :draggable="canEdit"
        ...
        <figcaption>
          ...
          <span v-else>第 {{ index + 1 }} 张</span></figcaption>
        <a-space v-if="canEdit" direction="vertical" :size="2">
          ...
          <a-space :size="2">
            <a-button size="small" :disabled="index === 0" :aria-label="`将图片 ${index + 1} 前移`"
                      @click="move(index, index - 1)">←
            </a-button>
            <a-button size="small" :disabled="index === modelValue.length - 1" :aria-label="`将图片 ${index + 1} 后移`"
                      @click="move(index, index + 1)">→
            </a-button>
          </a-space>
          <a-popconfirm title="从商品图集中移除此图片？" @confirm="remove(index)">
            <a-button size="small" danger>移除图片</a-button>
          </a-popconfirm>
        </a-space>
      </figure>
    </div>
...
const upload: UploadProps['customRequest'] = async options => {
  ...
  try {
    ...
    const response = await fileApi.uploadFile(data, 1) as unknown as ScmResponse<{
      fileKey: string;
      fileUrl: string;
      fileName?: string;
      fileSize?: number
    }>;
    if (!props.modelValue.some(image => image.fileKey === response.data.fileKey)) emit('update:modelValue', [...props.modelValue, {
      ...response.data,
      fileName: response.data.fileName || options.file.name,
      primaryFlag: props.modelValue.length === 0,
