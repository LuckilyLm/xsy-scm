...
<template>
  ...
  <a-table :data-source="rows" :columns="columns" row-key="tagId" :loading="loading" :pagination="false" size="small" bordered>
    <template #bodyCell="{ column, record }">
      <a-tag v-if="column.dataIndex === 'status'" :color="record.status === 'ENABLED' ? 'green' : 'default'">{{ enumLabel(ENABLE_STATUS_ENUM, record.status) }}</a-tag>
      ...
      <a-space v-else-if="column.dataIndex === 'action'" :size="0" class="smart-table-operate">
        <a-button v-privilege="'scm:product:tag:update'" type="link" size="small" @click="modal?.open(record)">编辑</a-button>
        <a-tooltip v-if="record.productCount > 0" title="仍有商品挂着此标签，只能停用；停用后不再出现在可选标签里，历史绑定保留">
          <a-button v-privilege="'scm:product:tag:delete'" type="link" size="small" disabled>删除</a-button>
        </a-tooltip>
        <a-popconfirm v-else title="确认删除此标签？" @confirm="remove(record)"><a-button v-privilege="'scm:product:tag:delete'" type="link" danger size="small">删除</a-button></a-popconfirm>
      </a-space>
    </template>
  </a-table>
...
const columns: TableColumnsType<ProductTag> = [
  { title: '排序', dataIndex: 'sortOrder', width: 70, align: 'right' }, { title: '标签编码', dataIndex: 'tagCode', width: 170 },
  { title: '标签名称', dataIndex: 'name', width: 170 }, { title: '已打标商品', dataIndex: 'productCount', width: 110, align: 'right' },
  { title: '状态', dataIndex: 'status', width: 80, align: 'center' }, { title: '操作', dataIndex: 'action', width: 140, align: 'right' },
];
async function load() {
  loading.value = true; error.value = '';
  try { rows.value = (await productTagApi.list({ ...query })).data; } catch (e) { error.value = productError(e); } finally { loading.value = false; }
}
...
async function remove(row: ProductTag) {
  try { await productTagApi.delete(row.tagId, row.version); message.success('标签已删除'); await load(); } catch (e) { error.value = productError(e); }
}
onMounted(load);
</script>
