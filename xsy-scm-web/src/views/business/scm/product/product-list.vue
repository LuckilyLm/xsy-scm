  ...
  <section aria-label="商品档案">
    <a-form class="smart-query-form" layout="inline" @finish="search">
      ...
      <a-form-item label="关键字" class="smart-query-form-item">
        <a-input v-model:value="filters.keyword" allow-clear placeholder="商品名 / 编码 / 条码 / 助记码"
                 style="width: 240px"/>
      </a-form-item>
      <a-form-item label="商品状态" class="smart-query-form-item">
        <a-select v-model:value="filters.status" allow-clear :options="SHELF_STATUS_ENUM" style="width: 110px"/>
      </a-form-item>
      ...
      <template v-if="advanced">
        <a-form-item label="SKU 状态" class="smart-query-form-item">
          <a-select v-model:value="filters.skuStatus" allow-clear :options="SHELF_STATUS_ENUM" style="width: 120px"/>
        </a-form-item>
        <a-form-item label="商品类型" class="smart-query-form-item">
          <a-select v-model:value="filters.productType" allow-clear :options="PRODUCT_TYPE_ENUM" style="width: 120px"/>
        </a-form-item>
        <a-form-item label="主档状态" class="smart-query-form-item">
          <a-select v-model:value="filters.masterStatus" allow-clear :options="MASTER_STATUS_ENUM"
                    style="width: 120px"/>
        </a-form-item>
        <a-form-item label="储存方式" class="smart-query-form-item">
          <a-select v-model:value="filters.storageMethod" allow-clear :options="STORAGE_METHOD_ENUM"
                    style="width: 110px"/>
        </a-form-item>
        <a-form-item label="标签" class="smart-query-form-item">
          <a-select v-model:value="filters.tagIds" mode="multiple" allow-clear :options="tagFilterOptions"
                    option-filter-prop="label" placeholder="命中任一标签" style="width: 220px"/>
        </a-form-item>
        <a-form-item label="主图" class="smart-query-form-item">
          <a-select v-model:value="filters.hasPrimaryImage" allow-clear :options="YES_NO_ENUM" style="width: 90px"/>
        </a-form-item>
        <a-form-item label="条码" class="smart-query-form-item">
          <a-select v-model:value="filters.hasBarcode" allow-clear :options="YES_NO_ENUM" style="width: 90px"/>
        </a-form-item>
        <a-form-item label="创建时间" class="smart-query-form-item">
          <a-range-picker v-model:value="createdRange" show-time value-format="YYYY-MM-DDTHH:mm:ssZ"
      ...
      <a-table :data-source="rows" :columns="columns" row-key="spuId" :loading="loading" :pagination="false"
        ...
        <template #bodyCell="{ column, record }">
          ...
          <span v-else-if="column.dataIndex === 'price'"
                class="price">{{ priceRange(record.minMarketPrice, record.maxMarketPrice) }}</span>
