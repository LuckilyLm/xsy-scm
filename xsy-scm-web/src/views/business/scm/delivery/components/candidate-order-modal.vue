<template>
  ...
  >
    ...
    <a-form layout="inline" class="candidate-filters" @submit.prevent="search">
      ...
      <a-form-item label="订单 / 客户 / 地址"><a-input v-model:value="query.keyword" allow-clear @pressEnter="search" /></a-form-item>
      ...
      <a-form-item label="金额"
        ><a-input-number v-model:value="query.minAmount" string-mode :min="0" placeholder="最低" /><span>至</span
        ><a-input-number v-model:value="query.maxAmount" string-mode :min="0" placeholder="最高"
      /></a-form-item>
      <a-form-item label="商品行数"
        ><a-input-number v-model:value="query.minItemCount" :min="0" :precision="0" placeholder="最少" /><span>至</span
        ><a-input-number v-model:value="query.maxItemCount" :min="0" :precision="0" placeholder="最多"
      /></a-form-item>
      <a-form-item><a-checkbox v-model:checked="query.locatedOnly">仅已定位</a-checkbox></a-form-item>
      <a-form-item
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'location'"
          ><a-tag :color="isLocated(record) ? 'green' : 'default'">{{ isLocated(record) ? '已定位' : '未定位' }}</a-tag></template
        >
        <template v-else-if="column.dataIndex === 'orderAmount'">{{ money(record.orderAmount) }}</template>
        <template v-else-if="column.dataIndex === 'expectDeliveryTime'">{{ datetime(record.expectDeliveryTime) }}</template>
      </template>
    </a-table>
    <div class="smart-query-table-page">
      <a-pagination v-model:current="query.pageNum" v-model:page-size="query.pageSize" :total="total" show-size-changer @change="load" />
    </div>
    <a-form layout="vertical"
      ><a-form-item label="组单原因" required><a-input v-model:value="reason" :maxlength="500" placeholder="例如：本次城区配送安排" /></a-form-item
    ></a-form>
