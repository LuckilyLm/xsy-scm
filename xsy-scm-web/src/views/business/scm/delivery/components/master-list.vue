<template>
  <a-form class="smart-query-form" layout="inline" @submit.prevent="search">
    <a-form-item :label="isDriver ? '司机编码' : '车牌号'"><a-input v-model:value="query.keyword" allow-clear @pressEnter="search" /></a-form-item>
    <a-form-item label="状态"><a-select v-model:value="query.status" allow-clear :options="statusOptions" class="status-select" /></a-form-item>
    <a-form-item
      ><a-space><a-button type="primary" @click="search">查询</a-button><a-button @click="reset">重置</a-button></a-space></a-form-item
    >
  </a-form>
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'status'"
          ><a-tag :color="record.status === 'ENABLED' ? 'green' : 'default'">{{ record.status === 'ENABLED' ? '启用' : '停用' }}</a-tag></template
        >
        <template v-else-if="column.dataIndex === 'action'"
          ><a-button type="link" v-privilege="editPermission" @click="open(record)">编辑</a-button></template
        >
      </template>
    </a-table>
  ...
  <a-modal v-model:open="visible" :title="`${form.id ? '编辑' : '新建'}${label}`" :confirm-loading="saving" @ok="save">
    ...
    <a-form layout="vertical">
      <template v-if="isDriver">
        <a-form-item label="司机编码" required><a-input v-model:value="form.driverCode" :maxlength="64" /></a-form-item>
        <a-form-item label="司机姓名" required><a-input v-model:value="form.driverName" :maxlength="100" /></a-form-item>
        <a-form-item label="联系电话" required><a-input v-model:value="form.phone" :maxlength="32" /></a-form-item>
      </template>
      <template v-else>
        <a-form-item label="车牌号" required><a-input v-model:value="form.vehicleNo" :maxlength="32" /></a-form-item>
        <a-form-item label="车型"><a-input v-model:value="form.vehicleType" :maxlength="64" /></a-form-item>
        <a-row :gutter="16"
          ><a-col :span="12"
            ><a-form-item label="载重（kg）"><a-input-number v-model:value="form.loadWeight" string-mode :min="0" :precision="4" /></a-form-item
          ></a-col>
          <a-col :span="12"
            ><a-form-item label="容积（m³）"
              ><a-input-number v-model:value="form.loadVolume" string-mode :min="0" :precision="4" /></a-form-item></a-col
        ></a-row>
      </template>
      <a-form-item label="状态"><a-select v-model:value="form.status" :options="statusOptions" /></a-form-item>
      <a-form-item label="备注"><a-textarea v-model:value="form.remark" :maxlength="500" :rows="2" /></a-form-item>
    </a-form>
  </a-modal>
</template>
