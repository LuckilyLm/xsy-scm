<template>
  <a-modal v-model:open="visible" title="线路发货单预览" :width="1000" ok-text="打印" :ok-button-props="{ disabled: loading || !data }" @ok="print">
    ...
    <a-spin :spinning="loading">
      <div v-if="data" ref="paper" class="delivery-print">
        ...
        <div class="print-meta">
          ...
          <p>
            司机：{{ data.detail.route.driverNameSnapshot || '未分配' }}　电话：{{ data.detail.route.driverPhoneSnapshot || '—' }}　车辆：{{
        ...
        <section v-for="stop in data.detail.stops" :key="stop.id">
          ...
          <p>{{ stop.addressSnapshot }} · {{ stop.receiverNameSnapshot || '—' }} · {{ stop.receiverPhoneSnapshot || '—' }}</p>
