<template>
  <a-drawer v-model:open="visible" title="配送线路详情" width="min(1500px, 96vw)" :destroy-on-close="true">
    <a-alert v-if="error" :message="error" type="error" show-icon
      ><template #action><a-button @click="reload">刷新线路</a-button></template></a-alert
    >
    <a-spin :spinning="loading">
      <template v-if="detail">
        <div class="route-heading">
          ...
          <a-space wrap>
            <a-tag :color="routeStatuses[detail.route.status].color">{{ routeStatuses[detail.route.status].label }}</a-tag>
            <a-button
              ...
              >编辑信息</a-button
            >
            <a-button v-if="detail.route.status === 'DRAFT'" type="primary" v-privilege="'scm:delivery:route:plan'" :disabled="busy" @click="plan"
              >确认规划</a-button
            >
            <a-button
        ...
        <a-tabs v-model:active-key="tab">
          <a-tab-pane key="base" tab="基础信息">
            <a-descriptions bordered :column="2" size="small">
              ...
              <a-descriptions-item label="仓库地址" :span="2">{{ detail.route.warehouseAddressSnapshot || '—' }}</a-descriptions-item>
              <a-descriptions-item label="司机"
                >{{ detail.route.driverNameSnapshot || '未分配' }} {{ detail.route.driverPhoneSnapshot }}</a-descriptions-item
              >
              <a-descriptions-item label="车辆">{{ detail.route.vehicleNoSnapshot || '未分配' }}</a-descriptions-item>
              <a-descriptions-item label="计划发车">{{ datetime(detail.route.plannedDepartureTime) }}</a-descriptions-item>
              <a-descriptions-item label="备注">{{ detail.route.remark || '—' }}</a-descriptions-item>
              <a-descriptions-item v-if="detail.route.cancelReason" label="取消原因" :span="2">{{ detail.route.cancelReason }}</a-descriptions-item>
            </a-descriptions>
          </a-tab-pane>
            ...
            >
              <template #bodyCell="{ column, record }">
                <template v-if="column.dataIndex === 'stop'"
                  >{{ stopOf(record.stopId)?.stopSeq }} · {{ stopOf(record.stopId)?.customerNameSnapshot }}</template
                >
                <template v-else-if="column.dataIndex === 'address'">{{ stopOf(record.stopId)?.addressSnapshot }}</template>
                <template v-else-if="column.dataIndex === 'orderAmountSnapshot'">{{ money(record.orderAmountSnapshot) }}</template>
                <template v-else-if="column.dataIndex === 'expectDeliveryTimeSnapshot'">{{ datetime(record.expectDeliveryTimeSnapshot) }}</template>
              ...
              <div class="route-stops">
                ...
                <ol>
                  ...
                  >
                    ...
                    <p>
                      {{ stop.orderCount }} 张订单 · {{ money(stop.totalAmount) }}<span v-if="stop.geomCrs"> · {{ stop.geomCrs }}</span>
                    </p>
                    ...
                    <a-space v-if="canEdit"
                      ...
                      ><a-button
                        ...
                        @click="move(index, index + 1)"
                        >下移</a-button
                      ><a-button size="small" :disabled="busy" @click="editStop(stop)">定位 / 备注</a-button></a-space
                    >
                  </li>
                </ol>
              </div>
  ...
  <a-modal
    ...
    @ok="submitReason"
  >
    <p>{{ reasonAction === 'cancel' ? '取消后，该线路的订单将重新进入待配送订单池。此操作不可撤销。' : '移除后，该订单可重新分配到其他线路。' }}</p>
    <a-form layout="vertical"
      ><a-form-item label="原因" required><a-textarea v-model:value="reason" :maxlength="500" :rows="3" /></a-form-item
    ></a-form>
  ...
  <a-modal v-model:open="stopVisible" title="停靠点定位与备注" :width="640" :confirm-loading="busy" @ok="saveStop">
    <template v-if="stopForm"
      ...
      <a-form layout="vertical">
        ...
        /></a-form-item>
        <a-form-item label="计划到达"><a-date-picker v-model:value="arrival" show-time value-format="YYYY-MM-DDTHH:mm:ssZ" /></a-form-item>
        <a-form-item label="备注"><a-textarea v-model:value="stopForm.remark" :maxlength="500" /></a-form-item> </a-form
...
<script setup lang="ts">
  ...
  const canEdit = computed(
    () =>
      ...
      (user.administratorFlag || user.getPointList?.some((point: { webPerms: string }) => point.webPerms === 'scm:delivery:route:update'))
  );
