<template>
  <a-form class="smart-query-form" layout="inline" @submit.prevent="search">
    <a-form-item label="配送日期"><a-date-picker v-model:value="query.deliveryDate" value-format="YYYY-MM-DD" /></a-form-item>
    <a-form-item label="线路"><a-input v-model:value="query.keyword" placeholder="线路名称 / 编号" allow-clear @pressEnter="search" /></a-form-item>
    <a-form-item label="状态"><a-select v-model:value="query.status" :options="statusOptions" allow-clear class="filter-select" /></a-form-item>
    ...
    <a-form-item label="车辆"
      ><a-select
        ...
        class="filter-select"
    /></a-form-item>
    <a-form-item label="省市区"><AreaCascader v-model:value="area" type="province_city_district" @change="changeArea" /></a-form-item>
    <a-form-item
      ><a-space
        ><a-button type="primary" @click="search" v-privilege="'scm:delivery:route:query'">查询</a-button
  ...
  <a-alert v-if="optionsError" :message="optionsError" type="warning" show-icon
    ><template #action><a-button @click="loadOptions">重载筛选项</a-button></template></a-alert
  >
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'status'"
          ><a-tag :color="routeStatuses[record.status as RouteStatus].color">{{ routeStatuses[record.status as RouteStatus].label }}</a-tag></template
        >
        <template v-else-if="column.dataIndex === 'coverage'"
          ><a-tag :color="record.locatedCount === record.stopCount && record.startGeomCrs && record.stopCount > 0 ? 'green' : 'orange'"
            >{{ record.locatedCount }} / {{ record.stopCount }}</a-tag
          ></template
        >
        <template v-else-if="column.dataIndex === 'totalAmount'">{{ money(record.totalAmount) }}</template>
        <template v-else-if="column.dataIndex === 'driverNameSnapshot'">{{ record.driverNameSnapshot || '未分配' }}</template>
        <template v-else-if="column.dataIndex === 'vehicleNoSnapshot'">{{ record.vehicleNoSnapshot || '未分配' }}</template>
        <template v-else-if="column.dataIndex === 'action'"
          ><a-space :size="0">
            ...
            <a-button v-if="record.status === 'DRAFT'" type="link" v-privilege="'scm:delivery:route:update'" @click="formDrawer?.open(record)"
              >编辑</a-button
            >
            <a-button
