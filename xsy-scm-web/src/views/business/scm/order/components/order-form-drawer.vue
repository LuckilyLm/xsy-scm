<template>
 <a-drawer :title="form.orderId?'编辑销售订单':'新建销售订单'" :open="visible" width="min(1200px, 96vw)" @close="visible=false">
  ...
  <a-spin :spinning="loading"><a-form :model="form" layout="vertical">
   ...
   <a-col :span="12"><a-form-item label="订单来源" name="orderSource"><SmartEnumSelect enum-name="SCM_ORDER_SOURCE_ENUM" v-model:value="form.orderSource" width="100%"/></a-form-item></a-col>
  ...
  <template #footer><a-space><a-button @click="visible=false">关闭</a-button><a-button type="primary" :loading="saving" :disabled="loading" @click="save">保存草稿</a-button></a-space></template>
 </a-drawer>
</template>
<script setup lang="ts">
import {ref} from 'vue';import {message} from 'ant-design-vue';
import CustomerSelect from '/@/components/business/scm/customer-select/index.vue';import SmartEnumSelect from '/@/components/framework/smart-enum-select/index.vue';
...
async function save(){error.value='';try{form.value.items.forEach(i=>{i.orderedQuantity=fixed(i.orderedQuantity);if(i.unitPrice!=null)i.unitPrice=fixed(i.unitPrice);});const invalid=validateOrder(form.value);if(invalid){error.value=invalid;return;}saving.value=true;const f=payload(form.value);if(f.orderSource!=='SUPPLEMENT'){f.originalOrderId=null;f.supplementReason=null;}await (f.orderId?orderApi.update(f):orderApi.create(f));message.success('草稿已保存');visible.value=false;emit('saved');}catch(e){error.value=orderError(e);}finally{saving.value=false;}}
defineExpose({open});
</script>
