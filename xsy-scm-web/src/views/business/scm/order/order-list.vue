...
<template>
 ...
 <a-card size="small" :bordered="false"><a-row class="smart-table-btn-block"><div class="smart-table-operate-block"><a-button type="primary" v-privilege="'scm:order:add'" @click="drawer?.open()">新建订单</a-button><a-button v-privilege="'scm:order:import'" @click="importModal?.open()">导入订单</a-button><a-button danger v-privilege="'scm:order:delete'" :disabled="!selected.length" @click="batchDelete">批量删除草稿</a-button></div><div class="smart-table-setting-block"><TableOperator v-model="columns" :table-id="602" :refresh="queryData"/></div></a-row>
 ...
 </a-card><OrderForm ref="drawer" @saved="queryData"/><OrderImportModal ref="importModal" @saved="queryData"/><OrderDetail ref="detail" @saved="queryData"/>
</template>
...
import {SCM_ORDER_STATUS_ENUM,SCM_ORDER_SOURCE_ENUM} from '/@/constants/business/scm/order-const';import type {Order,Query} from './order-types';import {amount} from './order-form-model';import {orderError} from './order-errors';import OrderForm from './components/order-form-drawer.vue';import OrderImportModal from './components/order-import-modal.vue';import OrderDetail from './order-detail.vue';
const queryForm=reactive<Query>({pageNum:1,pageSize:20}),tableData=ref<Order[]>([]),total=ref(0),loading=ref(false),error=ref(''),selected=ref<(string|number)[]>([]);const drawer=ref<InstanceType<typeof OrderForm>>(),importModal=ref<InstanceType<typeof OrderImportModal>>(),detail=ref<InstanceType<typeof OrderDetail>>();let requestId=0;
