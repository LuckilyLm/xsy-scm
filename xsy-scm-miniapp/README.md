...
1. **`mock/` 放在 `src/` 之外。** 这条规则的由来：早期把 mock 放在 `src/mock/` 时，
   产物里出现了 `dist/build/mp-weixin/mock/fixtures.js`，假数据被打进了包。
   事后用探针文件实测确认，**未引用的模块其实会被正常 tree-shake**（不会进产物），
   当时的真正根因是引入标识写成了 `@/mock`，被 uni-app 自带的 `@` → `src/` 别名截获、
   解析到 `src/mock/index.js`，**使整条 mock 依赖链变成可达**。
   也就是说：**根因是别名解析错误，不是「uni-app 会镜像整棵 src/ 树」**。
   两条规则现在都作为零成本纵深防御保留 —— 目录放外面，且引入标识用 `@mock`。
   隔离性已双向实测：生产构建中 `海岸城门店` / `本地小白菜` 等假数据命中数为 0。
